...
class TestWaitForRegistryAgent:
    ...
    Tests patch :meth:`SystemService._registry_auth_per_chain` (the multi-
    chain probe primitive) and feed it single-chain dicts; the aggregate
    string the wait loop computes from those mirrors the old single-chain
    return values for ``"ok"`` and is chain-prefixed for non-ok states.
    """
    ...
    async def test_returns_ok_immediately_when_check_succeeds(self, db):
        ...
        with patch.object(svc, "_registry_auth_per_chain", new=AsyncMock(return_value={"anvil": "ok"})):
    ...
    async def test_retries_past_agent_not_found(self, db):
        ...
        async def _probe():
            nonlocal call_count
            call_count += 1
            return {"anvil": "ok"} if call_count >= 2 else {"anvil": "agent_not_found"}

        with patch.object(svc, "_registry_auth_per_chain", new=_probe):
            result = await svc.wait_for_registry_agent(timeout=5.0)
        assert result["ready"] is True
        assert result["registry_auth"] == "ok"
        assert call_count >= 2
    ...
    async def test_retries_past_timeout(self, db):
        ...
        async def _probe():
            nonlocal call_count
            call_count += 1
            return {"anvil": "ok"} if call_count >= 2 else {"anvil": "timeout"}
    ...
    async def test_retries_past_unreachable(self, db):
        ...
        async def _probe():
            nonlocal call_count
            call_count += 1
            return {"anvil": "ok"} if call_count >= 2 else {"anvil": "unreachable"}
    ...
    @pytest.mark.asyncio
    async def test_retries_past_agent_not_resolved(self, db):
        """agent_not_resolved is the per-chain startup-task-in-flight state — must be retried."""
        svc = self._make_svc(db)
        call_count = 0
        ...
        async def _probe():
            nonlocal call_count
            call_count += 1
            return {"anvil": "ok"} if call_count >= 2 else {"anvil": "agent_not_resolved"}
    ...
    @pytest.mark.asyncio
    async def test_waits_for_all_chains(self, db):
        """A two-chain setup where one chain is pending must keep the wait alive."""
        svc = self._make_svc(db)
        call_count = 0
        ...
        async def _probe():
            ...
            call_count += 1
            if call_count == 1:
                return {"anvil": "ok", "base_sepolia": "agent_not_found"}
            return {"anvil": "ok", "base_sepolia": "ok"}

        with patch.object(svc, "_registry_auth_per_chain", new=_probe):
            result = await svc.wait_for_registry_agent(timeout=5.0)
        assert result["ready"] is True
        assert result["registry_auth"] == "ok"
        assert result["auth_per_chain"] == {"anvil": "ok", "base_sepolia": "ok"}
        assert call_count >= 2
    ...
    async def test_exits_immediately_on_owner_mismatch(self, db):
        ...
        Aggregate becomes ``"<chain>:owner_mismatch"`` so the operator
        knows which chain's identity is misconfigured.
        """
        svc = self._make_svc(db)
        call_count = 0
    ...
    async def test_returns_ready_false_on_timeout(self, db):
        ...
        svc = self._make_svc(db)
        with patch.object(svc, "_registry_auth_per_chain", new=AsyncMock(return_value={"anvil": "agent_not_found"})):
