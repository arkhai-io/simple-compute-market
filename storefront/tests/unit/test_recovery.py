...
def test_claim_happy_path_uses_local_attestation():
    order = _order()
    tag, params = derive_claim_params(order=order, payload={"listing_id": "ord-1"})
    assert tag == "ok"
...
def test_claim_order_not_found_returns_404():
    tag, status, body = derive_claim_params(order=None, payload={"listing_id": "gone"})
    assert tag == "error"
    assert status == 404
...
def test_claim_already_closed_returns_409():
    ...
    tag, status, body = derive_claim_params(order=order, payload={"listing_id": "ord-1"})
    assert tag == "error"
    assert status == 409
...
def test_claim_payload_override_wins_over_db():
    ...
    tag, params = derive_claim_params(
        order=order,
        payload={"listing_id": "ord-1", "fulfillment_uid": "0xfromclient"},
    )
    assert tag == "ok"
...
def test_reclaim_happy_path():
    ...
    tag, params = derive_reclaim_params(order=order, payload={"listing_id": "ord-1"})
    assert tag == "ok"
    assert params["escrow_uid"] == "0xescrow"


def test_reclaim_order_not_found_returns_404():
    tag, status, body = derive_reclaim_params(order=None, payload={"listing_id": "gone"})
    assert tag == "error"
    assert status == 404
...
def test_reclaim_terminal_state_returns_409(terminal):
    ...
    tag, status, body = derive_reclaim_params(order=order, payload={"listing_id": "ord-1"})
    assert tag == "error"
    assert status == 409
...
def test_arbitrate_happy_path_defaults_to_approve():
    order = _order()
    tag, params = derive_arbitrate_params(order=order, payload={"listing_id": "ord-1"})
    assert tag == "ok"
...
def test_arbitrate_decision_false_is_respected():
    ...
    tag, params = derive_arbitrate_params(
        order=order, payload={"listing_id": "ord-1", "decision": False}
    )
    assert tag == "ok"
...
def test_arbitrate_string_decision_is_normalized(decision_raw, expected):
    order = _order()
    tag, params = derive_arbitrate_params(
        order=order, payload={"listing_id": "ord-1", "decision": decision_raw}
    )
    assert tag == "ok"
...
def test_arbitrate_order_not_found_returns_404():
    tag, status, body = derive_arbitrate_params(order=None, payload={"listing_id": "gone"})
    assert tag == "error"
    assert status == 404
...
def test_arbitrate_without_attestation_and_no_override_returns_400():
    ...
    tag, status, body = derive_arbitrate_params(
        order=order, payload={"listing_id": "ord-1"}
    )
    assert tag == "error"
    assert status == 400
...
def test_arbitrate_payload_fulfillment_uid_overrides_db():
    ...
    tag, params = derive_arbitrate_params(
        order=order,
        payload={"listing_id": "ord-1", "fulfillment_uid": "0xoverride"},
    )
    assert tag == "ok"
