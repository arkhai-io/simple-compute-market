...
``close_market_order_endpoint``) live inside ``storefront/src/market_storefront/agent.py``, which
...
into a dedicated ``market_storefront/routes/`` package so they can be imported
...
_AGENT_ENV = {
    ...
    "TOKEN_REGISTRY_PATH": str(
        ...
        / "src/market_storefront/data/token_registry_docker_compose.json"
    ),
}
...
async def agent_app_client() -> AsyncIterator[AsyncClient]:
    ...
    from market_storefront.utils.event_ingestion import configure_default_ingestion
