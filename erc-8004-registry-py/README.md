...
All agents are identified using **ERC-8004 canonical IDs** following the format:
...
- `agentId`: Numeric ERC-721 tokenId assigned by the registry

**Important**: All agents must register on-chain first to receive an `agentId`. The canonical ID is derived from the on-chain registration and used consistently across all Indexer APIs.

### Breaking Changes

As of the latest update, API responses now use a single `agentId` field (canonical format) instead of separate `agentCanonicalId` and `onChainAgentId` fields. This ensures consistency with ERC-8004 specification.
...
**Required Fields**:
- `agentId`: ERC-8004 canonical ID (`eip155:{chainId}:{identityRegistry}:{agentId}`) - **REQUIRED**
- `agentCard` or `registrationFileUrl`: Agent card/registration file data
- `owner`: Wallet address of agent owner

**Note**: Agents must register on-chain first to receive an `agentId`. The canonical ID is derived from the on-chain registration. See [docs/WALLET_INTEGRATION.md](./docs/WALLET_INTEGRATION.md) for on-chain registration.
...
### Upgrading to Canonical ID Format

If you're upgrading from a previous version, note the following breaking changes:

1. **API Response Format**: All endpoints now return a single `agentId` field (canonical `eip155:...` format) instead of separate `agentCanonicalId` and `onChainAgentId` fields.

2. **Registration Requirement**: The `agentId` field is now **required** in registration requests and must be in ERC-8004 canonical format (`eip155:{chainId}:{identityRegistry}:{agentId}`).

3. **Database Migration**: Run the Alembic migration to update the schema:
   ```bash
   alembic upgrade head
   ```
   This will:
   - Drop the `agent_canonical_id` column
   - Transform `agent_id` to store canonical format
   - Add `onchain_agent_id` and `identity_registry` columns

4. **Agent Updates**: Update your agent code to:
   - Build canonical IDs after on-chain registration using format: `eip155:{chainId}:{identityRegistry}:{agentId}`
   - Pass canonical IDs to off-chain registration
   - Use canonical IDs in all API calls
