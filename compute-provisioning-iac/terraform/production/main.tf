...
# Installer Files Storage
module "installer_files_storage" {
  source             = "../common/modules/installer_files_storage"
  gcp_project_name   = var.gcp_project_name
  gcp_project_region = var.gcp_project_region
  resource_suffix    = ""
}

# Installer Files Storage Service Account with Role
module "installer_files_storage_rsa" {
  depends_on = [
    module.installer_files_storage
  ]
  source                 = "../common/modules/installer_files_storage_rsa"
  gcp_project_name       = var.gcp_project_name
  gcs_bucket_name        = module.installer_files_storage.bucket_name
  storage_sa_id          = "arkhai-installer-files-sa${local.name_suffix}"
  storage_sa_name        = "Installer Files Storage SA${local.name_suffix}"
  storage_sa_description = "Service account with Storage Object Creator and User roles for installer files bucket."
}
...
module "artifact_registry_reader_sa" {
  ...
  sa_id            = "arkhai-artifact-reader"
  sa_name          = "${var.gcp_project_name} Artifact Registry Reader"
