locals {
  ...
  bq_suffix   = var.resource_suffix != "" ? "_${replace(var.resource_suffix, "-", "_")}" : ""
}

resource "google_bigquery_dataset" "feedback_dataset" {
  project       = var.gcp_project_name
  dataset_id    = replace("${var.agent_name}_feedback${local.bq_suffix}", "-", "_")
  friendly_name = replace("${var.agent_name}_feedback${local.bq_suffix}", "-", "_")
  location      = var.gcp_project_region
}

resource "google_bigquery_dataset" "telemetry_logs_dataset" {
  project       = var.gcp_project_name
  dataset_id    = replace("${var.agent_name}_telemetry${local.bq_suffix}", "-", "_")
  friendly_name = replace("${var.agent_name}_telemetry${local.bq_suffix}", "-", "_")
  location      = var.gcp_project_region
}

resource "google_logging_project_sink" "feedback_export_to_bigquery" {
  name        = "${var.gcp_project_name}_feedback${local.bq_suffix}"
  project     = var.gcp_project_name
...
resource "google_logging_project_sink" "log_export_to_bigquery" {
  name        = "${var.gcp_project_name}_telemetry${local.bq_suffix}"
  project     = var.gcp_project_name
