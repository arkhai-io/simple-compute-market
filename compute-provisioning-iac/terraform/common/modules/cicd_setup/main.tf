...
resource "google_service_account" "cicd_runner_sa" {
  account_id   = "${var.gcp_project_prefix}${var.gcp_project_env}-cicd-sa${var.resource_suffix != "" ? "-${var.resource_suffix}" : ""}"
  display_name = "CICD Runner SA${local.display_suffix}"
  project      = var.cicd_gcp_project_name
}
...
resource "google_iam_workload_identity_pool" "github_pool" {
  workload_identity_pool_id = "${var.gcp_project_prefix}${var.gcp_project_env}-pool${var.resource_suffix != "" ? "-${var.resource_suffix}" : ""}"
  project                   = var.cicd_gcp_project_name
  display_name              = "GitHub Actions Pool${local.display_suffix}"
}

resource "google_iam_workload_identity_pool_provider" "github_provider" {
  workload_identity_pool_provider_id = "${var.gcp_project_prefix}${var.gcp_project_env}-oidc${var.resource_suffix != "" ? "-${var.resource_suffix}" : ""}"
  project                            = var.cicd_gcp_project_name
  workload_identity_pool_id          = google_iam_workload_identity_pool.github_pool.workload_identity_pool_id
  display_name                       = "GitHub OIDC Provider${local.display_suffix}"
  oidc {
...
resource "google_storage_bucket" "logs_data_bucket" {
  name                        = "${var.cicd_gcp_project_name}-cicd-logs${var.resource_suffix != "" ? "-${var.resource_suffix}" : ""}"
