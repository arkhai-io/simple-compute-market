"""Unit tests for service.identity (Phase 1 of pluggable-identity refactor)."""
...
from service.identity.registry import _VERIFIERS
from service.schemas import Identity

# Test keypair (anvil default account 1, well-known).
...
def test_register_identity_scheme_idempotent_for_same_object():
    verifier = get_identity_verifier("eip191")
    # re-registering the exact same object is a no-op
...
def test_eip191_verify_signature_rejects_non_matching_scheme():
    pytest.importorskip("eth_account")
    verifier = get_identity_verifier("eip191")
    # Address is right, but the scheme on the Identity says otherwise — the
    # verifier should refuse to validate identities it isn't authoritative for.
