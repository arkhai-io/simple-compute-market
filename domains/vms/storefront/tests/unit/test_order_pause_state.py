...
async def db(tmp_path) -> SQLiteClient:
    client = SQLiteClient(db_path=str(tmp_path / "test.db"), registry=build_vm_storefront_registry(build_vm_storefront_domain()))
...
class TestStartSyncNegotiationPauseGuard:
    ...
    async def test_global_pause_raises(self, db, monkeypatch):
        ...
        with pytest.raises(StorefrontPausedError) as exc_info:
            await start_sync_negotiation(registry=db.domain_registry, sqlite_client=db,
