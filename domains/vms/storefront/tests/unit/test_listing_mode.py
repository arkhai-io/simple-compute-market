...
class TestResolveVmListingMode:
    ...
    def test_explicit_fungible_recognized_regardless_of_structural_default(self):
        mode, explanation = resolve_vm_listing_mode(
            {"listing_mode": "fungible"}, structural_default="specific_resource",
        )
        assert mode == "fungible"
        assert explanation is None

    def test_explicit_specific_resource_recognized_regardless_of_structural_default(self):
        mode, explanation = resolve_vm_listing_mode(
            {"listing_mode": "specific_resource"}, structural_default="fungible",
        )
        assert mode == "specific_resource"
        assert explanation is None

    def test_unrecognized_value_falls_back_to_the_structural_default_with_explanation(self):
        mode, explanation = resolve_vm_listing_mode(
            {"listing_mode": "bogus"}, structural_default="fungible",
        )
        assert mode == "fungible"
    ...
    def test_unrecognized_value_falls_back_to_specific_resource_default_too(self):
        mode, explanation = resolve_vm_listing_mode(
            {"listing_mode": "bogus"}, structural_default="specific_resource",
        )
        assert mode == "specific_resource"
        assert explanation is not None
        assert "bogus" in explanation
