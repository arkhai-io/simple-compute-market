...
class TestHostsCsvImport:
    def test_imports_two_hosts(self, client, tmp_path):
        ...
        async def _run():
            report = await client.upsert_hosts_from_csv(
                csv_path=str(csv), dry_run=False
            )
...
class TestAdapterHostJoin:
    ...
    def test_from_domain_only_writes_slice_fields(self):
        ...
        db_row = adapter.from_domain_resource(
            resource, resource_id="r-1", state="available"
        )
...
class TestCapacityEnforcement:
    def _attrs(
        self, host: str, vcpu: int = 16, ram: int = 256, disk: int = 4000
    ) -> dict:
        return {
            "gpu_model": "H200",
            "sla": 99.0,
        ...
        async def _run():
            ...
            await client.upsert_resource(
                ...
                attributes=self._attrs(
                    "never-registered", vcpu=512, ram=4096, disk=80000
                ),
            )
            # Did not raise.
    ...
    def test_csv_import_surfaces_capacity_error_per_row(self, client, tmp_path):
        ...
        async def _run():
            ...
            report = await client.upsert_resources_from_csv(
                csv_path=str(resources), dry_run=False
            )
