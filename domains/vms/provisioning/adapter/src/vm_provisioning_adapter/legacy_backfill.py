...
def compile_legacy_vm_fulfillment_backfill(
    ...
    *,
    ...
    if target:
        # Deterministic so a re-run of this backfill against the same lease
        # always derives the same id -- but keyed on
        # candidate.capacity_reservation_id, not fulfillment_id. Unlike
        # FulfillmentConvergenceWatchdog._apply_create_success, which reads
        # an already-durable fulfillment_id off an existing row,
        # `fulfillment_id` here is freshly generated by the caller on every
        # invocation (see `_apply_legacy_vm_lease_backfill`) and is not
        # itself stable across re-runs. `capacity_reservation_id` is: it
        # comes from the pre-existing ledger scan this migration reads, not
        # from anything this compiler invents. Never the raw VM target
        # itself -- that would leave the fulfillment-owned identity
        # carrying provider-domain meaning that
        # `openspec/specs/fulfillment/architecture.md`'s "Fulfillment
        # result ownership" section says it must not.
        provisioned_resource_id = str(
            uuid.uuid5(
                uuid.NAMESPACE_URL,
                f"reservation:{candidate.capacity_reservation_id}:{target}",
            )
        )
    ...
    return LegacyFulfillmentBackfillDraft(
        ...
        provisioned_resource_ref=provisioned_resource_id if target else None,
    )
