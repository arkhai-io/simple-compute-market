...
class SQLiteClient(CoreSQLiteClient):
    ...
    async def load_agreement_site_route(
        self,
        *,
        negotiation_id: str,
    ) -> dict[str, str] | None:
        """Load the storefront-owned selected-site lifecycle correlation."""

        def _load() -> dict[str, str] | None:
            conn = sqlite3.connect(self.db_path)
            try:
                row = conn.execute(
                    "SELECT negotiation_id, site_id, capacity_reservation_id, "
                    "reserved_resource_id "
                    "FROM bare_metal_agreement_site_routing "
                ...
                return {
                    ...
                    "site_id": str(row[1]),
                    "capacity_reservation_id": str(row[2]),
                    "reserved_resource_id": str(row[3]),
                }
            finally:
                conn.close()
    ...
    async def record_agreement_site_route(
        self,
        *,
        negotiation_id: str,
        site_id: str,
        ...
        """Persist one immutable selected-site and reservation correlation."""
        ...
        def _save() -> dict[str, str]:
            ...
            try:
                ...
                existing = conn.execute(
                    "SELECT site_id, capacity_reservation_id, reserved_resource_id "
                    "FROM bare_metal_agreement_site_routing "
                    "WHERE negotiation_id = ?",
                    (negotiation_id,),
                ).fetchone()
                if existing is not None:
                    if existing != (
                        ...
                        raise ValueError(
                            "agreement already belongs to a different site or reservation",
                        )
                else:
                    conn.execute(
                        "INSERT INTO bare_metal_agreement_site_routing("
                        "negotiation_id, site_id, capacity_reservation_id, "
                        "reserved_resource_id"
                        ") VALUES (?, ?, ?, ?)",
                        (
                            negotiation_id,
