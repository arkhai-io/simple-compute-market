 ...
 * `adminKey` and talks to the credits service the same way the storefront
 ...
 *
 * This mirrors the Python reference (`apicredits_middleware.config`); the
 ...
 * `APICREDITS_MIDDLEWARE_`), recognising the same names as the Python
 * `GateConfig.from_env`.
 */
export function gateConfigFromEnv(
  env: NodeJS.ProcessEnv = process.env,
  prefix = "APICREDITS_MIDDLEWARE_",
): GateConfig {
