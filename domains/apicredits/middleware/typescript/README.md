# @arkhai/apicredits-middleware (TypeScript)

Seller-side gating middleware for the API-credits domain — the
TypeScript sibling of the Python reference
(`../python`). It extracts the bearer key from the `Authorization`
header, verifies it against the credits service (short-TTL cache), meters
...
```sh
npm install @arkhai/apicredits-middleware
```
...
import { tokenGateMiddleware, gateConfigFromEnv } from "@arkhai/apicredits-middleware";
...
```ts
import { withTokenGate, gateConfig } from "@arkhai/apicredits-middleware";
...
`gateConfigFromEnv()` reads `APICREDITS_MIDDLEWARE_*` variables
...
credits service — the gate only caches and meters.
...
in-process scripted credits service (`test/scriptedService.ts`) and
