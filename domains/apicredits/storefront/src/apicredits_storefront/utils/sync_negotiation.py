"""Synchronous request-response negotiation — API-credits flavor.
...
``{kind: "api_credits.v1", payload: {quantity, key}}`` fixed at round 0,
...
``credit_deal_terms`` row that settlement reads back for issuance.
"""
...
# inert for credit deals (the negotiated amount is always concrete).
...
def _compose_policy_catalogue() -> NegotiationCatalogue:
    ...
    policies obtained that way, so nothing in configuration can introduce them.
    """
    ...
    return compose_negotiation_catalogue(
        ...
        requested_policies=configured_policy_names(
            negotiation_config,
            default_chain=API_CREDITS_DEFAULT_SELLER_CHAIN,
...
async def _place_quota_hold(
    ...
    *,
    ...
    from apicredits_storefront.utils.config import settings as _settings
...
async def start_sync_negotiation(
    *,
    ...
    if quantity is not None:
        await sqlite_client.save_credit_terms(
            negotiation_id=neg_id,
...
async def continue_sync_negotiation(
    *,
    ...
    terms = await sqlite_client.load_credit_terms(negotiation_id=neg_id) or {}
