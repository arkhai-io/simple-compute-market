"""Polling-mode settlement for the API-credits domain.
...
issuance job against the credits service instead of VM provisioning;
...
async def start_settlement_job(
    *,
    ...
    from apicredits_storefront.services.claims_runtime import derive_claim_obligation
...
async def _run_settlement_job_bg(
    *,
    ...
    try:
        ...
        logger.exception(
            "[SETTLE_JOB] fulfill_credit_obligation raised for %s", escrow_uid,
        )
    ...
    if status == "fulfilled":
        ...
        if alkahest_client is not None:
            from apicredits_storefront.services.claims_runtime import submit_claim
