        ...
        while True:
            ...
            try:
                ...
                if status == "succeeded":
                    ...
                    try:
                        logs_response = await client.get(f"{base_url}/api/v1/jobs/{job_id}/logs", headers=headers)
...
async def schedule_vm_shutdown_async(
    ...
    This function:
    1. Submits a POST /api/v1/jobs request with vm_action='lease_end' and vm_lease_end time
    2. Receives a job_id
    3. Polls GET /api/v1/jobs/{job_id} until status is 'succeeded' or 'failed'
