...
CAPACITY_PROFILE_ROOT = PurePosixPath("tools/issue-discovery/config/capacity/profiles")
...
def _validate_seller_topology(
    ...
    *,
    ...
    expected_authority_mode = (
        "single-seller" if len(sellers) == 1 else "shared-globally-fenced"
    )
    ...
    if isinstance(declared_sellers, list) and set(seller_slots) != set(
        declared_sellers
    ):
        errors.append(
...
def _validate_requests(
    ...
    *,
    ...
    if isinstance(declared_buyers, list) and set(buyer_slots) != set(declared_buyers):
        errors.append(
            "requests must contain exactly one request for every declared buyer"
        )
...
def _validate_named_g1_shape(
    ...
    *,
    ...
    if scenario_id not in FROZEN_G1_SCENARIO_IDS:
        errors.append(
            "scenario_id is not part of the frozen fourteen-shape G1 envelope"
        )
        return
    ...
    for field_name, expected in (
        ...
        if scenario.get(field_name) != expected:
            errors.append(
                f"{field_name} must encode the exact O/B/S/H/L/R/G1 shape named by "
                "scenario_id"
            )
    ...
    *,
    ...
    if not errors:
        ...
        if scenario.get("gpu_assignment") != "whole-device-passthrough":
            errors.append(
                "current capacity scenarios require whole-device GPU passthrough"
            )
...
def _validate_repo_root(repo_root: Path) -> Path:
    ...
    try:
        ...
        raise CapacityValidationError(
            f"SCM repository root is unavailable: {error}"
        ) from error
    ...
    try:
        ...
        raise CapacityValidationError(
            f"cannot verify SCM repository root: {error}"
        ) from error
    if not same_root:
        raise CapacityValidationError(
            "repository root must be the exact SCM Git worktree root"
        )
    _reject_git_history_rewrite_state(top_level)
...
def _reject_git_history_rewrite_state(repo_root: Path) -> None:
    ...
    if replace_refs.strip():
        raise CapacityValidationError(
            "SCM authority rejects local Git history rewrite refs"
        )
    raw_grafts_path = (
        _run_git(repo_root, "rev-parse", "--git-path", "info/grafts")
        .decode("utf-8", errors="strict")
        .strip()
    )
    grafts_path = Path(raw_grafts_path)
    ...
    if (
        ...
        or grafts_path.read_bytes().strip()
    ):
        raise CapacityValidationError("SCM authority rejects local Git history grafts")
...
def _pinned_regular_blob(
    ...
    try:
        ...
        raise CapacityValidationError(
            "Git returned an invalid authority tree entry"
        ) from error
    if decoded_path != relative_path.as_posix():
        raise CapacityValidationError(
            "Git authority entry did not match the selected path"
        )
...
def _validate_registry_progression(
    ...
    *,
    ...
    if not isinstance(progression, dict):
        return
    if (
        tuple(progression.get("initial_buyer_order", ()))
        != MEASURED_INITIAL_BUYER_ORDER
    ):
...
def _validate_stage_admission(stage: dict[str, Any], *, errors: list[str]) -> None:
    ...
    actual_any_of = {frozenset(option) for option in any_of if isinstance(option, list)}
    ...
    *,
    ...
    if not errors:
        ...
        if registry.get("scenario_root") != CAPACITY_SCENARIO_ROOT.as_posix():
            errors.append(
                "profile scenario_root must name the public scenario directory"
            )
        ...
        if registry.get("non_counted_actors") != ["deterministic-controller"]:
            errors.append(
                "only the deterministic controller is non-counted orchestration"
            )
        if tuple(registry.get("frozen_scenario_ids", ())) != FROZEN_G1_SCENARIO_IDS:
            errors.append(
                "profile must freeze the exact ordered fourteen-scenario envelope"
            )
        ...
        if isinstance(stages, list):
            ...
            if set(stage_ids) != set(contracts):
                errors.append(
                    "profile must contain exactly the twenty-one frozen G1 stages"
                )
            for stage in stages:
...
def validate_profile_registry_file(
    ...
    *,
    ...
    try:
        ...
        raise CapacityValidationError(
            f"cannot read profile registry {path}: {error}"
        ) from error
    ...
    if stage_id == "b1-s1-g1-mock":
        ...
        if not isinstance(binding, dict):
            ...
            try:
                ...
                if not isinstance(physical, dict) or stage.get(
                    "independently_assignable_gpus"
...
def select_buyer_refinement_counts(
    ...
    while True:
        ...
        if not passing or not failing:
            ...
            expected_stage_id = (
                None if high - low <= 1 else f"b{(low + high) // 2}-s1-g1-measured"
            )
...
def retained_buyer_refinement_counts(
    ...
    selected_stage_ids = [f"b{count}-s1-g1-measured" for count in selected]
    missing = [
...
def buyer_frontier_is_lower_bound(
    ...
    *,
    ...
    if b8_stage not in stage_passes:
        raise CapacityValidationError(
            "buyer lower-bound authority requires the B8 result"
        )
...
def validate_buyer_refinement_sequence(
    ...
    missing = [
        stage_id for stage_id in expected_stage_ids if stage_id not in stage_passes
    ]
    if missing:
        raise CapacityValidationError(
...
def _validate_profile_stage_sequence(
    ...
    *,
    ...
    if len(sequence) != len(set(sequence)):
        raise CapacityValidationError(
            "profile stage sequence cannot contain duplicates"
        )
    ...
    if authority.raw_sha256 != pre_q0_registry_raw_sha256:
        raise CapacityValidationError(
            "profile registry raw bytes changed after Q0 began"
        )
    try:
...
def append_lifecycle(
    ...
    *,
    ...
    """Append one legacy finding-v1 lifecycle fact.

    Finding-v2 lifecycle is intentionally owned by the guarded publication
    capability. This helper remains only for the still-supported v1 issue
    repository paths.
    """
    event = {
        "schema_version": 1,
    ...
    with (run_dir / "issue-lifecycle.jsonl").open(
        "a",
        encoding="utf-8",
    ) as handle:
