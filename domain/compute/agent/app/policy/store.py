...
def ao_action_fulfill_after_accept(context: DecisionContext) -> DomainAction | None:
    ...
    if compute_buyer:
        ...
        # negotiation_id is set by the seller so the buyer can link escrow_uid to their local
        # order record. Fall back to order.order_id for backwards-compat (older seller agents).
        our_order_id = getattr(context.event, "negotiation_id", None) or order.order_id
        return DomainAction(
    ...
    return DomainAction(
        ...
        parameters={
            ...
            "counterparty_url": sender_url,
            "matched_order_id": matched_order_id,
            # negotiation_id is echoed by the buyer in their AcceptOfferEvent so the seller
            ...
            "negotiation_id": getattr(context.event, "negotiation_id", None) or order.order_id,
        },
    )
