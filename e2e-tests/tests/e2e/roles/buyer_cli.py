...
def create_profiled_buyer_cli(
    *,
    ...
    Domain scenarios supply only their public configuration sections. This
    ...
    registry_urls = tuple(
        dict.fromkeys(str(url).rstrip("/") for url in registries if str(url).strip())
    )
    if not registry_urls:
        raise ValueError(f"{domain_identity} registry list is empty")

    home_dir = base / "home"
    state_dir = base / "state"
    data_dir = base / "data"
    config_dir = base / "config" / "arkhai"
    for directory in (home_dir, state_dir, data_dir, config_dir):
        directory.mkdir(parents=True, exist_ok=True)

    signer = create_signer(marketplace_scheme, marketplace_credential)
    variable = credential_variable or (
        "ARKHAI_E2E_"
        + "".join(
            character if character.isalnum() else "_"
            for character in domain_identity.upper()
        )
        + "_MARKETPLACE_CREDENTIAL"
    )
    ...
    lines = [
        f"# Hermetic {domain_identity} buyer config generated by shared e2e fixtures.",
        "[BuyerProfile]",
    ...
    log.info(
        "[buyer_cli] domain=%s state=%s config=%s registries=%s",
        domain_identity,
        state_dir,
        config_path,
        registry_urls,
...
def buyer_cli(buyer_cli_binary: Path, tmp_path_factory) -> BuyerCli:
    """A VM buyer composed over the shared domain-neutral profile fixture."""
