...
**Current state.** The pool half exists; the listing half does not. A Resource Pool declares whether it is capacity-backed and which offering modes its listings may advertise, separately from the modes its provider can deliver, and [`ARCHITECTURE.md`](ARCHITECTURE.md) defines capacity-backed and unbacked for pools. There is still no notion of a listing that cannot be admitted against: the listing-level vocabulary is carried in [`unbacked-listing-publication`](../../openspec/changes/unbacked-listing-publication/)'s design until that change makes it true.
...
The durable listing binding carries a site and pool or Physical Resource provenance with no way to say which of those is an admission authority and which is merely where the listing came from. A Resource Pool can now authorize advertisement without proving delivery, and an unbacked pool — one that declares no deliverable mode, and so is kept out of every capacity path — can advertise modes on its own declaration, naming a configuration-free provider rather than fabricated configuration. Every existing pool was migrated as backed, advertising exactly what it delivers, and both declarations are required on every pool write and resolved by consumers through one shared resolver. Nothing reads them yet: no storefront derives, binds, or publishes a listing from them. A capacity declaration with no host behind it reaches storefronts: the resource-pool projection is built from declarations alone, while admission and placement still refuse one where the pool's provider delivers through a host. The compute registry schema carries no backing field, so a buyer cannot exclude supply nothing stands behind. A seller price is published today, but only inside the settlement carriers — the escrow and settlement-option rate structures — and no filter reads a rate value, so it cannot be compared on. Supply that settles by introduction carries no rate at all, because that mechanism's options are rateless by design. Rate comparison is therefore unavailable to backed and unbacked supply alike, for two different reasons.
