...
`kit/negotiation-runtime` and `kit/settlement-runtime`. The negotiation kit owns
signed round ordering, canonical-principal and terminal-state guards, durable
transcript recovery, and the acceptance chokepoint; VM and API-credit
storefronts inject their own listing resolution, codecs, seller policy,
configuration, accepted-artifact construction, and persistence/effect hooks.
Neither domain retains a lifecycle copy. The settlement kit owns one stable
per-obligation operation journal, conditional-escrow client port, servicing
worker, and failure dispatcher composed by VM and API credits. Bare metal
composes exact verified-plan registration and adoption but truthfully remains
...
The remaining cross-cutting copies are capacity, publication, negotiation
watchdog, and chain-client construction. Those concerns still make a new
storefront domain larger than its codecs, contract, configuration, and genuine
domain actions.
