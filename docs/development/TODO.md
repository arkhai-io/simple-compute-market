...
- The market skeleton still lives across the VM buyer domain package and top-level `storefront/`, tangled with compute code → extract `market-core` so the package graph expresses the joint the `run_buy(...)` signature already implies.
- **Package migration target:** move the remaining top-level executable packages into the `core/`, `kit/`, `domains/vms/` framework only after the import graph matches the target boundaries. `domains/vms` should be organized by VM market concepts (`listings/`, `negotiation/`, `settlement/`, `provisioning`) plus thin concrete executable packages (`buyer/`, `storefront/`), rather than mirroring core roles. The policy implementation split is done: generic chain machinery lives in `kit/policy/`, while VM negotiation/settlement policies live under `domains/vms/negotiation/`. VM models, resource adapters, resource CSV import, compute listing reconciliation, listing filters/rendering/price/strategy helpers now live under `domains/vms/listings/`; accepted-escrow selection, proposal materialization, compute lease encoding, token materialization, Alkahest escrow creation, and post-provisioning fulfillment submission live under `domains/vms/settlement/`; VM capacity checks, provision-term construction, fulfillment-plan construction, provisioning job-spec construction, provisioning-service client helpers, fulfillment orchestration, and the provisioning-service executable package now live under `domains/vms/provisioning/`; the VM buyer CLI package now lives under `domains/vms/buyer/`; the default VM storefront seller-round hook now lives under `domains/vms/negotiation/`; schema-agnostic registry publication fan-out now lives in `market_core.registry_publication`; and the registry/storefront protocol client packages plus schema-agnostic registry service now live under `core/` while preserving their import and wheel names. The next concrete prerequisites are the remaining top-level `service/` and `storefront/` package moves.
