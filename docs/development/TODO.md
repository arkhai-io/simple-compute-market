...
- **Package migration target:** move the remaining top-level executable packages into the `core/`, `kit/`, `domains/vms/` framework only after the import graph matches the target boundaries. `domains/vms` should be organized by VM market concepts (`listings/`, `negotiation/`, `settlement/`, `provisioning/`) rather than mirroring core roles. The next concrete prerequisites are: split `policy/` into generic chain machinery vs VM negotiation/settlement policies; cut the buyer schema-plugin boundary; extract storefront hook interfaces around `sync_negotiation.py` and `action_executor.py`; then move `provisioning-service` under `domains/vms/provisioning` and registry/client packages under `core/` with temporary compatibility wrappers.
