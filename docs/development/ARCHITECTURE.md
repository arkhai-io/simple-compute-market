...
- **From below** — concrete, mutually-independent utilities: negotiation middlewares (`market-policy`), identity schemes (`market-identity`), generic schemas (`market-core`), infra clients (alkahest, chain, registry-client). The defining property is "depended on, never depending up into the skeleton" — not "packaged as one wheel."
