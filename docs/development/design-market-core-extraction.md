...
| from-below kit (existing, stays split) | `market-policy` (middlewares), `market-identity` (identity models + verifiers), `market-core` (generic schemas + orchestration-neutral helpers), `market-service` (compatibility exports + infra clients), `registry-client` / `storefront-client`. Mutually independent utilities.                            | nothing in the skeleton |
