...
| from-below kit (existing, stays split) | `market-policy` (middlewares), `market-identity` (identity models + verifiers), `market-service` (generic schemas + infra clients), `registry-client` / `storefront-client`. Mutually independent utilities.                            | nothing in the skeleton |
