...
from app.policies.decorators import requires_trigger, requires_event_type
from app.policies.action_builders import NegotiationActionBuilder, ResourceActionBuilder
...
# Consolidated from 4 policies → 2 policies for better maintainability

# Negotiation configuration
NEGOTIATION_CONFIG = {
    "min_factor": 0.8,           # Reservation band min (80% of our price)
    "max_factor": 1.2,           # Reservation band max (120% of our price)
    "reasonable_min": 0.5,       # Reasonable range min (50% of our price)
    "reasonable_max": 2.0,       # Reasonable range max (200% of our price)
    "max_rounds": 5,             # Maximum negotiation rounds
    "timeout_seconds": 300,      # Timeout in seconds (5 minutes)
}


# Helper functions for negotiation logic
def _is_valid_price_data(our_price: Any, their_price: Any) -> bool:
    """Validate price data for negotiation."""
    return (
        our_price is not None
        and their_price is not None
        and isinstance(our_price, (int, float))
        and isinstance(their_price, (int, float))
        and our_price > 0
        and their_price > 0
    )


def _check_exit_conditions(history: list[dict[str, Any]], config: dict[str, Any]) -> str | None:
    """Check all exit conditions, return reason if any met."""
    # Max rounds
    if len(history) >= config["max_rounds"]:
        return "max_rounds"

    # Staleness (no price movement)
    if len(history) >= 2:
        last_two_prices = [
            msg.get("proposed_price") or msg.get("their_price")
            for msg in history[-2:]
        ]
        if last_two_prices[0] == last_two_prices[1]:
            return "stale_negotiation"
    ...
    if history:
        ...
        if first_ts:
            try:
                ...
                elapsed = (datetime.now(first_time.tzinfo) - first_time).total_seconds()
...
def _calculate_midpoint(our_price: int, their_price: int, history: list[dict[str, Any]], agent_id: str) -> int:
    """Calculate counter-offer price as midpoint between last proposal and their price."""
    last_our_proposal = our_price
    if history:
        for msg in reversed(history):
            if msg.get("sender") == agent_id and msg.get("proposed_price"):
                last_our_proposal = msg.get("proposed_price")
                break
    return (last_our_proposal + their_price) // 2


@policy_callable("negotiation.action.price_aware_decision")
@requires_trigger("negotiation")
@requires_event_type(NegotiationEvent)
def price_aware_decision(context: DecisionContext) -> DomainAction | None:
    """Single policy for price-based negotiation decisions.

    Combines entry guard (price equality check) with core decision logic.

    CGT role: Implements "Entry Decision → Negotiation Game" handoff and
    per-round strategic decisions using reservation bands and price history.

    Returns:
        ACCEPT_OFFER if price equal or within band,
        COUNTER_OFFER with proposed price if outside but reasonable,
        EXIT_NEGOTIATION if far outside our band,
        None if data invalid (passes to safety policy)
    """
    data = context.event.data or {}
    our_price = data.get("our_price")
    their_price = data.get("their_price")

    # Safety check - let termination_conditions handle invalid data
    if not _is_valid_price_data(our_price, their_price):
        return None
    ...
    # Entry guard: Accept immediately if prices equal
    ...
    # Core decision: Evaluate price bands
    min_price = our_price * config["min_factor"]
    max_price = our_price * config["max_factor"]

    # Determine if we're buying or selling based on resources
    order = data.get("our_order") or {}
    offer_resource = order.get("offer_resource", {})
    is_selling = "gpu_model" in offer_resource  # We're offering compute

    # Within reservation band → accept
    if is_selling:
        if their_price >= min_price:  # Selling: they pay at least our minimum
            logger.info(f"[NEGOTIATION] Their price {their_price} >= min {min_price}, accepting")
            return actions.accept("within_band")
    else:
        if their_price <= max_price:  # Buying: we pay at most our maximum
            logger.info(f"[NEGOTIATION] Their price {their_price} <= max {max_price}, accepting")
            return actions.accept("within_band")

    # Outside band but reasonable → counter-offer
    reasonable_min = our_price * config["reasonable_min"]
    reasonable_max = our_price * config["reasonable_max"]

    if reasonable_min <= their_price <= reasonable_max:
        proposed = _calculate_midpoint(our_price, their_price, negotiation_history, context.agent_id)
        logger.info(f"[NEGOTIATION] Counter-offering {proposed} (between last proposal and {their_price})")
        return actions.counter(proposed)

    # Far outside reasonable range → exit
    logger.info(f"[NEGOTIATION] Their price {their_price} outside reasonable range [{reasonable_min}, {reasonable_max}], exiting")
    return actions.exit("price_unreasonable")


@policy_callable("negotiation.guard.termination_conditions")
@requires_trigger("negotiation")
@requires_event_type(NegotiationEvent)
def termination_conditions(context: DecisionContext) -> DomainAction | None:
    """Single policy for all termination conditions.

    Combines round/timeout limits with safety checks for invalid data.

    CGT role: Encodes terminal checks and TIMEOUT/FAILURE outcomes, plus
    safety fallback for error conditions.

    Returns:
        EXIT_NEGOTIATION if limits exceeded,
        REJECT_OFFER if data invalid,
        None to continue negotiation
    """
    data = context.event.data or {}
    our_price = data.get("our_price")
    their_price = data.get("their_price")
    ...
    # Safety check: Reject if data invalid
    if not _is_valid_price_data(our_price, their_price):
        logger.warning(f"[NEGOTIATION] Invalid price data, rejecting for safety")
        return actions.reject("invalid_price_data")

    # Check all exit conditions
    exit_reason = _check_exit_conditions(negotiation_history, config)
    if exit_reason:
        logger.info(f"[NEGOTIATION] Exiting due to: {exit_reason}")
        return actions.exit(exit_reason)

    return None  # No termination, continue
