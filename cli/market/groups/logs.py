"""CLI commands for inspecting stage logs and deal status.

    market logs                          # all stage events, most recent first
    market logs --negotiation <id>       # events for one negotiation
    market logs --stage settlement       # events for one stage
    market logs --last 5                 # last 5 events

    market status <negotiation_id>       # derive current stage from DB state
"""
