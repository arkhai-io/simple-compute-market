"""Adapter over SyncAgentClient for the smoke test suite.
...
This adapter translates those conventions to the canonical ``SyncAgentClient``
interface without any ``asyncio.run()`` indirection — ``SyncAgentClient`` is
fully synchronous.

To remove this adapter: update ``test_agents.py`` to:
  1. Import ``SyncAgentClient`` from the wheel directly
...
from agent_client import SyncAgentClient, AgentClientError  # noqa: F401

__all__ = ["AgentClient", "AgentClientError"]


class AgentClient:
    """Smoke-test adapter over SyncAgentClient.

    Preserves the interface expected by test_agents.py while delegating
    all HTTP work to the canonical SyncAgentClient.
    """
