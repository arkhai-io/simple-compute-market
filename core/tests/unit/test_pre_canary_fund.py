...
def test_pre_canary_fund_plans_ethereum_sepolia_topups_from_shared_rpc(
    tmp_path: Path,
) -> None:
    module = _load_script_module()
    shared_secrets_dir = tmp_path / "shared-secrets"
    ...
    _write_env(
        ...
        {
            "CANARY_TOKEN_SYMBOL": "USDC",
            "CANARY_TOKEN_AMOUNT": "1.25",
            "CANARY_DURATION_HOURS": "1",
            "BUYER_NATIVE_FLOOR_WEI": "20000",
            "SELLER_NATIVE_FLOOR_WEI": "10000",
            "BUYER_TOKEN_BUFFER_BASE_UNITS": "50000",
        },
    )
    ...
    plan = module.build_funding_plan(
        ...
        erc20_balances={
            (context.buyer_wallet_address, token_metadata.address): 0,
        },
    )

    assert context.chain_name == "ethereum_sepolia"
    assert context.rpc_url == "https://alchemy.example/eth-sepolia-http"
    assert context.funder_private_key == "0xshared-funder-private-key"
