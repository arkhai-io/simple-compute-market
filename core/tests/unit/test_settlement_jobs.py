...
from core.agent.app.utils.settlement_jobs import (
...
async def test_start_happy_path_inserts_row_and_kicks_off_task(client):
    ...
    with patch(
        "core.agent.app.utils.settlement_jobs._run_settlement_job_bg",
        new=AsyncMock(),
    ):
...
async def test_background_task_writes_ready_on_success(client):
    ...
    with patch(
        "core.agent.app.utils.action_executor.fulfill_compute_obligation",
        new=mock_fulfill,
    ):
