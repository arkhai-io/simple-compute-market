...
| Domain-neutral hint keys (`listing_mode`, `max_reservation_hold_seconds`, later extended to `region`/`sla`/`pricing`) live in `kit/resource-pools`; domains own accepted values/defaults; `listing_mode` values are `"fungible"`/`"specific_resource"` -- formalizes the pooled-vs-specific-resource decision `available_compute_slices` already made implicitly from local table membership. **Superseded by the corrected, fully-current row below** (this row's own anchors predate the `"pooled"`→`"fungible"` terminology fix and the requirement's later rename; kept struck rather than silently deleted, matching this table's own no-silent-edits convention) | superseded — see `openspec/specs/resource-pool-management/spec.md#domain-neutral-publication-and-hold-hints`, promoted 2026-08-04, below |
| `compute_capacity_pools`/`compute_pool_members` are live-read (not orphaned) by `domains/vms/listings/reconciler.available_compute_slices` | **Superseded, not promoted separately (2026-08-06):** every requirement Sections 4-6 promoted for `available_compute_slices`'s actual behavior already depends on and re-demonstrates this fact (e.g. "Commercial mapping identity," "Domain-owned publication and hold hints"); a standalone "this table isn't dead" requirement would just restate context those already carry. |
| Local physical-authority retirement is column-level for `resources`/`compute_capacity_pools` (both hybrid physical+commercial), table-level for `hosts`/`compute_pool_members`; freeze-then-redirect, `DROP` deferred to a follow-up cleanup change | **Deferred, not this change's promotion to make (corrected 2026-08-06):** Section 6 re-scoped the actual retirement work out of this change entirely -- see task 6.4 and `design.md`'s "Splitting Section 6 across two changes." This decision belongs to that future change's own design/promotion record, not a `storefront-publication` requirement this change finalizes. |
| CSV import (`host_csv_importer.py`/`resource_csv_importer.py`) and `upsert_resource` are removed, not repurposed, once the projection is the physical-registration source | **Deferred, not this change's promotion to make (corrected 2026-08-06):** same re-scoping as the row above -- CSV import isn't removed by this change at all now; see task 6.4. |
...
| `use_site_projection_for_listings` defaults `true` -- the local-table fallback path has parity with the projection path as of Section 5; retiring the fallback path and the flag itself is out of this change's own scope, deferred to a separately-proposed follow-up change with no assumed trigger condition this document can define | `openspec/specs/storefront-publication/spec.md#storefronts-cache-independent-site-projections` — **promoted 2026-08-05, corrected to a real anchor 2026-08-06 (that requirement extended with a "projection-backed derivation has reached parity" scenario for exactly this decision, replacing a `#local-physical-authority` anchor that was never actually created)** |
