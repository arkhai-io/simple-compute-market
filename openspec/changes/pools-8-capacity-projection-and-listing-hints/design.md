...
### What `listing_cardinality_mode` actually needs to formalize
...
| `AggregateCapacityClient.reserve(site=...)` dispatches to `_reserve_at_site`/`_reserve_by_placement`; site-pinning applies to every listing with a known site mapping regardless of `listing_cardinality_mode`, never only resource-pinned ones | `openspec/specs/storefront-publication/spec.md#site-pinned-claim-routing` — **promoted 2026-08-04** |
| Domain-neutral hint keys (`listing_cardinality_mode`, `max_reservation_hold_seconds`, later extended to `region`/`sla`/`pricing`) live in `kit/resource-pools`; domains own accepted values/defaults; `listing_cardinality_mode` values are `"fungible"`/`"specific_resource"` -- formalizes the pooled-vs-specific-resource decision `available_compute_slices` already made implicitly from local table membership | Superseded by the corrected row below — `openspec/specs/resource-pool-management/spec.md#domain-neutral-publication-and-hold-hints`, promoted 2026-08-04 |
| `compute_capacity_pools`/`compute_pool_members` are live-read (not orphaned) by `domains/vms/listings/reconciler.available_compute_slices` | Not promoted separately — subsumed by the Sections 4-6 requirements below that already depend on and demonstrate this |
| Local physical-authority retirement is column-level for `resources`/`compute_capacity_pools`, table-level for `hosts`/`compute_pool_members`; freeze-then-redirect, `DROP` deferred to a follow-up cleanup change | Deferred — this change's retirement scope re-homed to `pools-9-retire-local-physical-authority`; that change's own promotion record, not this one's |
| CSV import (`host_csv_importer.py`/`resource_csv_importer.py`) and `upsert_resource` are removed, not repurposed, once the projection is the physical-registration source | Deferred — same re-homing as the row above, `pools-9-retire-local-physical-authority` |
...
| `listing_cardinality_mode` values are `"fungible"`/`"specific_resource"` in code as well as prose (an earlier draft's code sketch still said `"pooled"`, corrected 2026-08-04) | `openspec/specs/resource-pool-management/spec.md#domain-neutral-publication-and-hold-hints` — **promoted 2026-08-04** |
...
| `use_site_projection_for_listings` defaults `true` -- the local-table fallback path has parity with the projection path as of Section 5; retiring the fallback path and the flag itself is out of this change's own scope, deferred to `pools-9-retire-local-physical-authority` | `openspec/specs/storefront-publication/spec.md#storefronts-cache-independent-site-projections` — **promoted 2026-08-05** |
| The three-tier region/SLA/pricing precedence applies regardless of whether a pool has a local `compute_capacity_pools` row -- a missing storefront-override tier removes only that tier, never the pool's ability to resolve and publish through its own hint or the storefront's config default | `openspec/specs/storefront-publication/spec.md#domain-owned-publication-and-hold-hints` — **promoted 2026-08-04, behavior corrected to match 2026-08-05** |
