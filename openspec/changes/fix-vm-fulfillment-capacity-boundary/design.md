...
Field-name translation between the capacity/negotiation vocabulary (`gpu_count`/`vcpu_count`/`ram_gb`/`disk_gb`) and the Ansible-facing vocabulary (`vm_gpu_count`/`gpu_provisioned`/`vm_vcpus`/`vm_ram`/`vm_disk_size`) lives in `arkhai_vms` (`domains/vms/domain`), already a dependency of both `domains/vms/storefront` and `domains/vms/provisioning/adapter`. No new shared package is needed.
