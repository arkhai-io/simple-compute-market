...
| [`managed-bare-metal-lifecycle`](managed-bare-metal-lifecycle/) | active; design accepted, section 1 account/endpoint/trust hardening accepted and its rules promoted to the owning specifications, sections 2-6 not started; automatic host reuse gated on physical qualification | One serialized, generation-fenced bare-metal release that confines tenant access and persistent writes, revokes TPM-sealed recoverable lease storage by counter, reboots, verifies, and quarantines on uncertainty, with confirmed relisting. No payment changes |
