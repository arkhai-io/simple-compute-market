...
| [`unbacked-listing-publication`](unbacked-listing-publication/) | unblocked: `pool-declared-advertisement-and-backing` is archived, and this change wires its shared declaration resolver into storefront projection ingestion; `settle-listing-vocabulary` is archived and `project-capacity-resources-without-hosts` is complete; plus a completion dependency on `pools-9-retire-local-physical-authority` | Backing as an explicit declared listing property: a tagged union over admission provenance, a binding discriminator distinct from the listing's origin site, pool advertise-authorization separated from execute-authorization, capacity-availability reconciliation scoped to backed listings while source-publication reconciliation applies to all, and an exact backing filter in the compute registry schema. Design revised 2026-09-23: listing identity is the physical resource offered and terms of sale refresh in place, the seller's inventory guard checks each listing against its own source, older producers are detected jointly per site with unresolvable pools held, and publication stops using `derived_compute_listings`; re-planning pending |
