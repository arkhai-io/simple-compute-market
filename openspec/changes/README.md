...
| [`bare-metal-dev-demo-run`](bare-metal-dev-demo-run/) | integrated Alkahest seller collection passed final bounded Spec and Standards review plus precommit owning, buyer, and offline E2E checks; exact committed-generation artifact/image qualification, commit, and live gates remain; two unrelated baseline release-tooling failures remain | Constrains whole-host account admission and access, composes validated accepted settlement, confirmed publication, and exact seller-collection lifecycle behavior, and retains contract/image, actual-host, funding, token-transfer, cleanup, and financial closeout evidence as separate live gates |
