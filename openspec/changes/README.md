...
| Change | Status | Acceptance boundary |
|---|---|---|
| [`publish-multidimensional-listing-shape`](publish-multidimensional-listing-shape/) | implemented; awaiting code review, promotion, and end-to-end evidence | Every VM listing is a listing shape — the storefront's site-scoped override, else the pool's `listing_shapes` hint, else the GPU-only default generator — published and reserved at exactly its declared quantities, and only where a source member is feasible for it; so the registry's dimension filters match stated shapes. Adds site-scoped storefront pool overrides, administered through an authenticated API checked against the site's live projection. Implements the family-grouped shape form, its shared utility (`kit/capability-shape`), and the VM family schema from `structured-capacity-requirements` |
