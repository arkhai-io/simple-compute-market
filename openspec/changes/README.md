...
| [`bare-metal-dev-demo-run`](bare-metal-dev-demo-run/) | implemented; awaiting review | Constrains the operating-system account a whole-host access action may name, persists a tenant-facing endpoint distinct from the provisioner's, and renders chain configuration for the whole-host storefront under Alkahest |
