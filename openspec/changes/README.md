...
```text
unify-host-identity (archived) ──► capacity-resource-administration ──► pools-9-retire-local-physical-authority
...
| [`capacity-resource-administration`](capacity-resource-administration/) | implemented, end-to-end pipeline passed, promoted; second-review corrections (Section 7c) planned before archival | Site capacity resources become the single authoritative declaration of sellable capacity across every dimension, with a digest-gated startup import and an always-reconciling REST import, a composition-supplied mirror dimension, a required `pool_id` with a pool-reassignment drain rule, and derivation of declarations from legacy host GPU columns at INI seed time and once at upgrade. Planned; host correlation is the declaration's `host_id` from `unify-host-identity` |
| [`pools-9-retire-local-physical-authority`](pools-9-retire-local-physical-authority/) | planned; its dependency `capacity-resource-administration` is complete, archival pending | Retires every remaining physical-resource concern from the VM storefront: local physical-authority tables, `compute_allocations`, CSV import and its deployment contract, the orphaned physical admin surface, and the always-`None` `vm_host` plumbing. The deployment-bake trigger for its own start remains undefined by design; the dependency is a necessary gate, not a sufficient one |
...
```text
unify-host-identity (archived) ──► capacity-resource-administration ──► project-capacity-resources-without-hosts ──┐
...
`capacity-resource-administration` is a Goal 7 prerequisite and delivers what Goal 7
relies on: digest-gated capacity-definition import, a composition-supplied mirror
...
| [`project-capacity-resources-without-hosts`](project-capacity-resources-without-hosts/) | ready to plan; its dependency `capacity-resource-administration` is complete, archival pending | Inverts the resource-pool projection to iterate declared capacity resources and correlate host rows in, so a declaration with no executor host reaches storefronts instead of succeeding into a void. Also serves Goal 1 |
