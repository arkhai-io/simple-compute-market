...
| [`managed-bare-metal-lifecycle`](managed-bare-metal-lifecycle/) | active; design, section 1 account/endpoint/trust hardening and the 2.1 rendering checkpoint accepted; the 2.2 simulator custody and supervision seams are verified, and one simulated systemd/helper/ESAPI/regular-file LUKS preparation and retry composition passed and its bounded checkpoint review accepted; production storage activation and overall integration remain pending; sections 2.3–2.6 and 3–6 remain open; automatic host reuse is gated on physical qualification | One serialized, generation-fenced bare-metal release that confines tenant access and persistent writes, revokes TPM-sealed recoverable lease storage by counter, reboots, verifies, and quarantines on uncertainty, with confirmed relisting. No payment changes |
