...
repair-multi-storefront-scenario ───────────────────────────────────────────────┴──► pools-9-retire-local-physical-authority
fix-vm-fulfillment-capacity-boundary ──► retire-vm-remove-job-id
remove-dead-storefront-physical-surfaces, fix-resource-pool-provider-at-creation (independent; split from pools-9 2026-09-25)
```
...
| [`pools-9-retire-local-physical-authority`](pools-9-retire-local-physical-authority/) | planned; re-grounded 2026-09-25. `capacity-resource-administration` is archived; its cutover now also depends on `repair-multi-storefront-scenario` (below, under the local end-to-end stack), because the two-storefront scenario's second storefront derives from the local tables this change deletes | Retires the local-table listing path and its flag, the local physical-authority tables, CSV import and its deployment contract, and the legacy home-site storefront override tier with its writer — with no carry-over of legacy values into the site-scoped store `publish-multidimensional-listing-shape` delivered, which is the only override tier afterwards. The endpoint it once planned is delivered by that store. The dead surfaces and the provider rule it accumulated are split out below. The deployment-bake trigger for its own start remains undefined by design |
| [`remove-dead-storefront-physical-surfaces`](remove-dead-storefront-physical-surfaces/) | planned; split from `pools-9` 2026-09-25; no blocking dependency | Retires the storefront's zero-caller physical surfaces: the `compute_allocations` execution ledger (frozen, not dropped), the always-`None` `vm_host` plumbing, the orphaned resource admin routes with their client methods, the legacy half of `release_reservations`, four dead `SQLiteClient` methods, and `resource_count` on the health surface. Removes code only; the terminal requirement stays `pools-9`'s |
| [`fix-resource-pool-provider-at-creation`](fix-resource-pool-provider-at-creation/) | planned; split from `pools-9` 2026-09-25; no blocking dependency | A Resource Pool's provider is fixed at creation: replace and patch refuse a differing provider without deleting existing configuration, matching the immutable backing declaration; inventory moves executors through a second pool and member migration, safe under `capacity-resource-administration`'s drain invariant |
| [`fix-vm-fulfillment-capacity-boundary`](fix-vm-fulfillment-capacity-boundary/) | complete; closeout finished 2026-09-25; ready to archive | Removes stale physical-placement fields from the current fulfillment path and derives fulfillment shape from committed reservation dimensions. Also serves Goal 2. Proven by a green e2e run on 2026-09-14. Its one deferral, retiring the `vm_remove_job_id` mirror, is owned by `retire-vm-remove-job-id` below |
...
| [`compose-contact-exchange-across-compute`](compose-contact-exchange-across-compute/) | blocked on `contact-payload-retention`; design-complete. Owns the system evidence for unbacked listings (its 6.4 and 6.5, transferred from the archived `unbacked-listing-publication`) | Promotes the domain-neutral introduction composition glue out of bare metal so accepted-state interpretation has one implementation, composes the mechanism in the VM storefront — the one remaining compute-family domain — extends delivery to it, and resolves the seller's contact payload per listing origin rather than per storefront. Goal 7's multi-seller introduction value depends on that last part |
...
bare-metal-publication-reads-pool-declarations (archived) ──► bare-metal-listing-shapes ──► unbacked-bare-metal-listings
...
| [`bare-metal-mock-provisioned-deal`](bare-metal-mock-provisioned-deal/) | design decided; not planned; unblocked: the bare-metal end-to-end lane it runs on exists | A mock-provisioned whole-host deal on the bare-metal lane on every pipeline run — discovery, negotiation, Alkahest settlement, delivery through the mock, teardown, and the site's capacity returned — through typed clients. Gives the bare-metal adapter its own mock (bare-metal execution runs inside the VM adapter today, so it first needs a seam), and the bare-metal storefront lifecycle pause and step controls. Supplies `bare-metal-and-credits-domain-stacks`' pipeline deal evidence; real access and revocation stay the protected lane's |
| [`bare-metal-listing-shapes`](bare-metal-listing-shapes/) | design phase; not planned; unblocked. Next on Goal 7's critical path | Each bare-metal listing carries a capability shape matching its Physical Resource, published where the compute schema's dimension filters read it, so bare-metal supply is discoverable by hardware; bare metal joins the site-scoped pool-override store. The key per-shape asking rates are declared against |
| [`unbacked-bare-metal-listings`](unbacked-bare-metal-listings/) | design phase; not planned; blocked on `bare-metal-listing-shapes` and `compose-contact-exchange-across-compute` Sections 1–3b | Bare metal publishes unbacked listings from unbacked pools through the same kit runtime, binding, and reconciliation as VM, settling by introduction through the promoted composition. Owns the bare-metal system evidence that unbacked discovery reaches a usable introduction |
...
| [`relay-vm-access-without-a-dashboard`](relay-vm-access-without-a-dashboard/) | implemented and promoted (142 of 170 tasks); live-hardware verification (sections 6 and 8), decision gate 1A.7, and closeout outstanding; row corrected 2026-09-25, it had still said "in design" | VM tunnel allocation and verification stop depending on a relay dashboard, DNS name, certificate, and second credential. A relay becomes an administered resource with its own controller, holding its own window and encrypted token, changeable against a running service rather than by redeployment, and no longer reverted when a pod restarts against an unchanged definition document. The host's management and buyer tunnel clients are split, and adding a VM stops restarting the tunnel client — and with it every buyer's live session |
