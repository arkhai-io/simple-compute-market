...
| Change | Status | Acceptance boundary |
|---|---|---|
| [`settle-listing-vocabulary`](settle-listing-vocabulary/) | implemented; every changed boundary carries in-process integration coverage; the e2e harness gap that blocked its multi-service scenario is resolved and the suite runs green, leaving two system-level assertions unrun and the provisioning contract's field rename outstanding; independent | One name for the offering mode across the claim wire, pool declarations, the durable binding, and the published listing — retiring `executor_kind`, `virtualization_type`, and the proposed `offering_type`. `offer_resource` becomes `listing_resource` and `offer` returns to meaning a negotiation message. The compute schema identity names its family. `executor` keeps only its action-dispatch meaning; its uses as a synonym for the machine, the handler, or the mode are retired. The cardinality hint keeps its deprecated ingestion alias on the read path; the wire renames get none. Three column renames across three databases, two payload backfills, and an advanced provisioning contract version; see [`cutover.md`](settle-listing-vocabulary/cutover.md) for the coordinated deploy |
...
| [`managed-bare-metal-lifecycle`](managed-bare-metal-lifecycle/) | active; design, section 1 account/endpoint/trust hardening and the 2.1 rendering checkpoint accepted; the 2.2 simulator custody and supervision seams are verified, one simulated systemd/helper/ESAPI/regular-file LUKS preparation and retry composition passed, one native disposable guest proved the bounded synthetic loop/mapper/ext4/PID 1 mount/checked-cleanup prerequisite, and one isolated synthetic prepared-secret activation passed through the uninstalled production activation supervisor/owner with live resources retained until guest disposal; ordinary role preparation, installation, physical-device and reboot-recovery integration remain pending; sections 2.3–2.6 and 3–6 remain open; automatic host reuse is gated on physical qualification | One serialized, generation-fenced bare-metal release that confines tenant access and persistent writes, revokes TPM-sealed recoverable lease storage by counter, reboots, verifies, and quarantines on uncertainty, with confirmed relisting. No payment changes |
