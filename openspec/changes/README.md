...
| [`bare-metal-listing-shapes`](bare-metal-listing-shapes/) | design decided; not planned; unblocked. Next on Goal 7's critical path | Each bare-metal listing carries a capability shape matching its Physical Resource, published where the compute schema's dimension filters read it, so bare-metal supply is discoverable by hardware; bare metal joins the site-scoped pool-override store. The key per-shape asking rates are declared against |
