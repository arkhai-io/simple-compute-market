...
| [`publish-indicative-listing-rates`](publish-indicative-listing-rates/) | blocked on `unbacked-listing-publication`; one decision gate open (rate asset), delta partial | A seller's asking rate for a listing's advertised shape, with the period drawn from the canonical time-unit vocabulary — `hour` only, matching what the VM and bare-metal domains support. Filters match the period rather than normalizing across it. Normatively a listing attribute rather than a settlement option rate: nothing is constructed from it. Closes Goal 7's comparison gap |
