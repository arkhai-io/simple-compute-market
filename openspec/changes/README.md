...
| [`bare-metal-dev-demo-run`](bare-metal-dev-demo-run/) | final closeout candidate: implementation and committed artifacts are qualified; the controlled campaign ended in verified failed-payment refund, physical release, and owned cleanup after seller collection failed; seller payment and several replacement-only physical assertions remain unproved; repository/evidence disposition remains | Constrains whole-host account admission and access, composes validated accepted settlement, confirmed publication, and exact seller-collection lifecycle behavior, while recording the actual failed-payment outcome without presenting it as seller-payment or payment-independent E2E success |
