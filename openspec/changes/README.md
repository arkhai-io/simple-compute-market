...
| [`bare-metal-dev-demo-run`](bare-metal-dev-demo-run/) | implemented; scoped offline qualification reviewed; two unrelated baseline release-tooling failures remain | Constrains whole-host account admission and access, composes validated accepted settlement and confirmed publication lifecycle behavior, and retains operator-controlled actual-host execution and financial closeout as separate gates |
