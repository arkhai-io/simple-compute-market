...
| [`add-host-ssh-port`](add-host-ssh-port/) | planned | The host registry records the SSH port the provisioner connects on, `ansible_port` survives INI import instead of being silently discarded, and both inventory renderers emit it. Takes no position on where a port value comes from |
