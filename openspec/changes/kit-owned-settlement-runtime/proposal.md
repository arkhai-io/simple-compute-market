...
Settlement job orchestration, claim servicing, and failure handling are duplicated
across the VM and API-credits storefronts — `settlement_jobs` at 368 against 274 lines,
`claims_runtime` at 224 against 128, and `failure_policy` at 392 against 182. Bare metal
has none of them and therefore cannot complete a deal.

These three are grouped because they are one lifecycle: a settlement job orchestrates,
claims servicing advances obligations, and failure policy decides what happens when a
step fails. Extracting one without the others would leave the seam cutting through the
middle of a control flow.

The duplication is also where new settlement capability keeps landing unevenly. Generic
per-obligation lifecycle, interval escrows, and seller bonds are all in flight; each will
otherwise be built once per domain.
