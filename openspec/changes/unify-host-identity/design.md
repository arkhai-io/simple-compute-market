...
**Amended after code review, 2026-09-21:** the first implementation did not keep
this promise on SQLite; see "Review outcomes" below.
...
- ~~**Distribution version bumps are deferred** to one bump after
  `capacity-resource-administration`.~~ **Reversed after code review;** see
  "Review outcomes".

### Review outcomes (2026-09-21)

A code review found three blockers and several gaps; the owner accepted the
following. Each is implemented.

- **The migration was not atomic on SQLite.** The Python driver opens a
  transaction only before a data statement, so DDL issued first inside
  `engine.begin()` ran outside any transaction and survived a failure. The
  migration now plans every rewrite — reading and validating every row — before
  its first write, and runs inside `_schema_transaction`, which disables the
  driver's own transaction management and opens the transaction explicitly so
  `ALTER TABLE` rolls back with the data. The capacity-declaration rebuild uses
  the same helper, with foreign-key enforcement disabled on the same
  connection. Tests start from `fixtures/schema_through_20260911_001.sql`, the
  schema the preceding chain actually produces (generated by running it), and
  a failure test compares every schema object and every row.
- **JSON rewrites are path-specific.** A recursive rename would have rewritten
  any nested key with a retired name — including an operator's
  `provider_extra_vars` entry named `machine_id`. Each stored shape now has its
  own rewrite at the exact location that names the host; a test proves an
  operator variable of that name survives.
- **The bare-metal reset would have orphaned registry listings.** Listing ids
  are random and only the storefront's database tracks them, so deleting that
  database left the registry's listings open with nothing able to close them.
  The owner's procedure replaces it: terminate leases, `helm uninstall` the
  bare-metal release and delete its volumes, deploy, reinstall. The provisioning
  service is shared with VM and is migrated, never reset. Listings in a registry
  shared with VM are closed first by disabling the bare-metal declarations and
  running one publish round, which closes every tracked listing on an
  authoritative empty projection.
- **Consumers of the generic rename were missed.** The census searched for the
  specialized spellings but not for `HostResponse.name`; seven e2e accesses
  would have failed at run time. A call-site audit of every host-returning
  method now covers it. A static type check was tried as further evidence and
  rejected: reintroducing `host.name` deliberately produced no diagnostic,
  because the client's return types reach the scenarios untyped.
- **Versions are bumped in this change.** It can be packaged and deployed on its
  own, and changed wire contracts under unchanged versions would let stale
  producer and consumer combinations resolve. Packages whose contract changed
  take a minor bump; a package whose metadata changes only because a
  dependency's bound moved takes a patch bump; a consumer's bound moves only
  where it depends on the changed contract. Locks were regenerated with only the
  bumped packages upgraded, avoiding unrelated movement. The VM storefront's lock
  could not be regenerated in the implementation environment — its `rl` extra
  resolves `torch` for macOS from an index that environment cannot reach — so
  its bumped entries were taken from locks `uv` regenerated from the same wheels,
  and verified by a frozen install and a full suite run.
- **The capacity integration helper uses the canonical clients.** It now
  delegates to `SiteCapacityAdminClient` and `SiteCapacityClient` and builds no
  request of its own; raw HTTP remains only in documented rejection-path tests.
  Doing so exposed that the client's `commit` returns nothing, so the lifecycle
  test reads the committed reservation back through the client.
- **Out of scope, filed as follow-up:** the bare-metal lease endpoints have no
  canonical client, so `test_bare_metal_leases_api.py`'s test-local wrapper is a
  client-contract gap predating this change. Fixing it means deciding which
  client owns those endpoints.
- **Storefront client parity is guarded in full.** The service suite now
...
- the bare-metal deployment is **reset**, not migrated, following the manual
  procedure in `tasks.md` task 4.7 (amended after review: the whole bare-metal
  release is uninstalled and its volumes deleted, rather than only the
  storefront database — see "Review outcomes");
