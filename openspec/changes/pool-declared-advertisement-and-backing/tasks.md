...
- [x] 1.1 Add `advertisable_modes` to the shared policy-tag vocabulary with the same
...
- [x] 1.2 Make an empty advertisable declaration authorize no mode, never widened
      by a default. Files: `RP/hints.py`.
- [x] 1.3 Add `capacity_backing` with values `backed` and `unbacked`, as module
...
- [x] 1.3a Fix `capacity_backing` at creation. `replace_pool` and `update_pool`
      ...
      Files: `RP/service.py`.
- [x] 1.3b Require both tags on every write. Add a `model_validator(mode="after")`
...
- [x] 1.3c Implement one shared validation returning structured problems
      ...
      `RP/service.py`.
- [x] 1.3d Canonical export emits `policy_tags` verbatim, so with 1.3b every
...
- [x] 1.3e Name each problem's path in the error document import raises: join
...
- [x] 1.4 Carry both tags through create, replace, patch, bulk import, projection,
...
- [x] 1.5 Leave `deliverable_modes` untouched — meaning, derivation, migration, and
...
- [x] 1.6 Update the module docstring in `RP/hints.py` to describe both new tags,
...
- [x] 2.1 Enforce on write that a pool declaring `capacity_backing: backed` has an
...
- [x] 2.2 Expose `resolve_pool_declarations(policy_tags) -> PoolDeclarations`, a
...
- [x] 2.3 Leave an `unbacked` pool's advertisable set independent of its deliverable
...
- [x] 2.4 Enforce on write, and in the resolver, that a pool declaring
...
- [x] 3.1 Add `Migration("20260922_001_pool_advertisement_and_backing", ...)` at the
...
- [x] 3.1a Do the same for the API-credits service: append a migration to
...
- [x] 3.3 Confirm no existing deployment's advertising surface or admission
...
- [x] 4.0 Move `kit/resource-pools/tests/unit/test_resource_pool_service.py`, which
...
- [x] 4.1 **Unit.** Exhaustive declaration and resolver cases for both tags: valid,
...
- [x] 4.1a **Integration (library).** The same rule set asserted identically through
...
- [x] 4.2 **Integration.** An operator creates an unbacked pool declaring an
      ...
      `PT/integration/test_pools_api.py`.
- [x] 4.3 **Integration.** Through the canonical pool client: create, replace, patch,
...
- [x] 4.3a **Integration.** A document predating the declarations fails validation
...
- [x] 4.4 **Integration.** A backed pool's widened advertisable declaration, and a
      narrowing of its deliverable set below it, are each rejected on write.
      Files: `PT/integration/test_pools_api.py`.
- [x] 4.5 **Integration.** A malformed backing value is rejected on write, and an
      unbacked pool with a non-empty deliverable set is rejected on write.
      Files: `PT/integration/test_pools_api.py`.
- [x] 4.5a **Integration.** A replace, patch, or imported entry changing an existing
...
- [x] 4.6 **Migration suite.** Migration derives both values on a database written
...
- [x] 4.8 Run the resource-pool kit, fulfillment kit, provisioning service, and
...
- [x] 4.9 **Executor-default inventory.** `docs/development/TESTING.md` makes this
...
- **Kit** (`market_resource_pools`): validation, cross-tag rules, resolver
  (`resolve_pool_declarations`, `PoolDeclarationError`,
  `MissingPoolDeclarationError`), model validators, service immutability,
...
- **API credits**: migration `20260922_004_pool_advertisement_and_backing`,
  declared `default` seed, load-time check. `make test`: 34 passed; `src/tests`:
  32 passed.
- **Other suites against the rebuilt wheel**: `kit/fulfillment` 176 passed,
  `kit/site` 254 passed, `e2e-tests/tests/unit` 237 passed with 1 failure
...
- **4.9 inventory**: the only production reads of either new tag are the two
  backing-immutability checks in `service.py`, where an absent stored value only
  permits supplying one (the repair path) and never resolves to a backing. Both
...
- [x] 5.1 **Comment hygiene.** Run `make check-comment-hygiene` and resolve every
...
- [x] 5.2 **Import placement.** Review imports this change added or touched and
...
- [ ] 5.3 **Documentation compliance.** Re-check accepted decisions against
      ...
      rather than leaving the box unchecked. **Disposition:** the second mode
      declaration belongs in the permanent map. `ARCHITECTURE.md`'s
      one-name-per-concept paragraph already names "the Resource Pool's
      deliverable and advertisable declarations", which this change makes true;
      the authority-boundaries table gains an advertisement-authorization and
      backing row beside the deliverable-mode row, and the "Resource pools"
      section gains one paragraph. These edits, and the promotions below, land
      with 5.7 after code review, per `AGENTS.md`. Also promote:
...
- [x] 5.8 **Documentation citations.** Run
