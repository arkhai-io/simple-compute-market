...
- [ ] 12.4 Verify the implementation against every POOLS-7 scenario and archive the OpenSpec change after validation. `proposal.md`'s "Permanent documentation impact" checklist (task 7.14) was added during Section 7 review rather than deferred here; confirmed it still reflects reality against the final Section 8-11 state. **Blocked, not merely unstarted:** this change cannot archive without a passing end-to-end result, which does not exist yet -- the test harness has not successfully mounted and submitted a result in the environment where that's attempted, independent of anything in this review pass (this review's own environment has no live docker-compose stack to attempt it in either). This is also the same live-services confirmation task 10.14 flagged as still outstanding after its otherwise-complete static/structural correction. Do not archive on the strength of 12.1-12.3's documentation work or this review's unit/integration suite results alone -- 11.18.5 and this review pass both explicitly waived strict OpenSpec validation's absence and later ran it for real (see 8.15), but no equivalent path exists yet for the e2e gate; it stays open until a real result exists. **A real result now exists: run 31608431467 (2026-08-12), 98 passed / 12 skipped / 0 failed against a live docker-compose stack, with the POOLS-7 teardown chain among the passing stages.** The remaining work in this task is archival itself — `openspec archive`, which synchronizes this change's spec deltas into `openspec/specs/` — and strict OpenSpec validation, both of which are repository operations rather than evidence-gathering. Left checked-off pending that operation rather than marked done by it.
