...
- Section 9 (storefront orchestration cutover) is implemented at the source level, tasks 9.0–9.8 complete; its design-promotion record is `design.md`'s "Section 9 completed design-promotion record," promoted into `openspec/specs/fulfillment/spec.md`, `openspec/specs/compute-provisioning-contract/spec.md`, and `docs/development/ARCHITECTURE.md`. A subsequent review found the implementation persists restart-recovery identifiers without yet implementing restart recovery itself — see `tasks.md`'s Section 9 status note and the open discuss-phase questions it records; Section 9 is not considered closed pending that resolution. Sections 10–11 (teardown/reclamation and obsolete-schema removal) have not started; their promotion records do not exist yet and will be added as those sections implement.
