...
**Status note (2026-07-31, Section 11 second code review):** the matcher and
foreign-key rebuild blockers are resolved, but Section 11 remains under review.
Tasks 11.14–11.18 reopen bounded work for a non-empty API-credit migration
sequence, composition-owned credits clients, direct HTTP-contract tests, SQLite
identifier safety, wheel/import isolation, and documentation reconciliation. A
separate migration deployment topology, startup-lifecycle testing, new client
protocols, and core-layer lifecycle/watchdog refactors are explicitly out of
scope for this correction. Permanent design promotion remains pending review
acceptance.
