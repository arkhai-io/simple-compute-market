...
**Status note (2026-08-01, Section 11 second code review):** the matcher and foreign-key rebuild blockers from the first review round are resolved, but Section 11 remains under review. A second review round accepted tasks 11.14–11.18 (a non-empty API-credit migration sequence, composition-owned credits clients with direct HTTP-contract tests, SQLite identifier safety for the generic table-rebuild helper, wheel/import isolation, and documentation reconciliation) and explicitly rejected expanding into a deployment-topology redesign, startup-lifecycle tests, a new client protocol, capacity-admin caller expansion, and core-layer refactors. The review upload marked 11.16.1, 11.16.3, 11.17.2, 11.18.1, 11.18.3, and 11.18.4 complete, but each was checked directly against the actual files and found unchanged from before the review; it also carried two accidental regressions from being generated against a stale base (predating this change's own deletion of `domains/apicredits/settlement/issuance.py`): a dead, duplicate-`CreditsServiceError` copy of that file reappeared unused by anything, and this paragraph was briefly reverted to an earlier draft of itself. Both reconciled. Every falsely-claimed item was then implemented for real: 11.16's comment/docstring tightening plus a strict SQLite identifier validator on the generic table-rebuild helper (with injection-shaped rejection tests); 11.17.2's domain-scoped editable-source-rejection test; 11.18.1's correction of task 11.10's own record, which had claimed "deployment init wiring" and "deployment-init contract tests" that were never built and were later explicitly excluded from scope by 11.14.3; and 11.18.4's removal of speculative future-deployment-topology language from `db/migrations.py`. 11.17.3 additionally surfaced a genuine, previously-undiscovered issue: the storefront Dockerfile's runtime stage did `COPY domains/ ./domains/`, meaning `domains.apicredits.*` resolved from a raw source copy rather than the properly wheel-installed `arkhai-apicredits-domain` package. Verified before fixing that the wheel already contains everything the storefront actually imports (checked its force-include list and grepped for any file-path-based, non-import access), removed the redundant copy, and validated the fix as rigorously as a sandbox without a Docker daemon allows: built every transitive wheel, installed them into a clean venv with only the storefront's own source present, and confirmed every `domains.apicredits.*` import the storefront performs resolves from site-packages — then turned that verification into a permanent regression test rather than a one-off check. An actual `docker build` was not performed; this remains the one piece of 11.14–11.18 without a fully independent build-and-run confirmation. 11.18.3 (shortening every one of 11.1–11.13's completed-task notes, moving detailed alternatives and debugging narratives into `design.md` first where not already there) and 11.18.6 (the Section 11 design promotion record) are now complete as of 2026-08-01 — see `tasks.md`'s "Section 11 design promotion record" for the destinations. **Section 11 is complete.** Full suite: 1,470+ tests across every touched package, zero failures.
