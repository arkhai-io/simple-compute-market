...
- [ ] 1.1 Add `ansible/roles/vm-setup/tasks/passthrough-audit.yml`. Read-only:
...
- [ ] 1.2 Identify the host-critical devices in the same task file: the PCI
...
- [ ] 1.3 Classify each GPU group. Bindable when every member is the GPU
...
- [ ] 1.4 Fail closed on IOMMU state. Empty `/sys/class/iommu/` or an empty
...
- [ ] 1.5 Emit a structured audit fact — per GPU: address, model, group number,
...
- [ ] 1.6 Add `ansible/playbooks/host-kit/passthrough-audit.yaml` targeting
...
- [ ] 1.7 Register the new playbook in `Makefile`'s `validate-playbooks` target
...
- [ ] 2.1 Add `tests/test_passthrough_audit.py` using `extract_between`,
      `fake_binaries`, and `run_bash`. Fake `lspci`, `ip`, `findmnt`, `lsblk`,
      and a temporary sysfs tree for `/sys/kernel/iommu_groups` and
      `/sys/bus/pci/devices`.
- [ ] 2.2 Cover, each as its own case: GPU with audio function only → bindable;
...
- [ ] 2.3 Cover the incident's exact topology as a named regression case: GPU
...
- [ ] 2.4 Cover fail-closed: empty `/sys/class/iommu/` produces "cannot
      assess" and never produces an empty-and-therefore-clean bind list.
- [ ] 2.5 Cover host-critical identification with several interfaces present,
...
- [ ] 3.1 Rewrite `ansible/roles/vm-setup/tasks/gpu-passthrough.yml` to consume
...
- [ ] 3.2 Remove `vfio-pci.ids=` and `vfio-pci.disable_vga=1` from the GRUB
      command-line edit. The IOMMU token (`intel_iommu=on` / `amd_iommu=on`)
      stays: it binds no device and cannot strand the host.
- [ ] 3.3 Remove the vendor-ID list from `/etc/modprobe.d/vfio.conf`. The task
...
- [ ] 3.4 Keep `/etc/modules-load.d/vfio.conf`. Loading `vfio_pci` binds
      nothing on its own once no `ids=` is supplied, and the module must be
      present before an override is applied.
- [ ] 3.5 Add `ansible/roles/vm-setup/templates/vfio-bind.sh.j2` and
      ...
      than code.
- [ ] 3.6 Make the script a no-op when `/proc/cmdline` carries the rescue
      token. This is what makes the rescue entry inert without a second
      initramfs.
- [ ] 3.7 Bind failure must fail the unit and leave the host running. No
      `ignore_errors` on the apply task, and no reboot triggered by a bind
      outcome.
- [ ] 3.8 Confirm the existing `blacklist-nvidia.conf` task and the existing
...
- [ ] 4.1 Add `ansible/roles/vm-setup/tasks/rescue-boot-entry.yml` installing a
...
- [ ] 4.2 The entry is written once and never regenerated from current state.
...
- [ ] 4.3 Set `GRUB_DEFAULT=saved` and leave the normal entry as default.
...
- [ ] 4.4 Include the rescue entry's title and selection procedure in the audit
...
- [ ] 5.1 Include `passthrough-audit.yml` from
...
- [ ] 5.2 Include `rescue-boot-entry.yml` after `gpu-passthrough.yml`.
- [ ] 5.3 Confirm the role's closing reboot is now reachable only for the IOMMU
...
- [ ] 5.4 Report skipped GPUs prominently in the role's closing debug output —
...
- [ ] 6.1 Trace whether anything infers GPU capacity from host preparation
...
- [ ] 6.2 **Decide and record** in `design.md` whether an unbindable GPU must be
      ...
      whichever is chosen.
- [ ] 6.3 Implement the decision from 6.2, or record explicitly that no code
...
- [ ] 8.1 **Comment hygiene.** Run `make check-comment-hygiene` from the
...
- [ ] 8.2 **Import placement.** Applies to `tests/test_passthrough_audit.py`
...
task file. It is not touched by this change and is named here only so the
observation is not lost; removing it is its own decision.
