...
|---|---|
| Pinned scenario authority and schema-v2 VM/GPU semantics | `openspec/specs/capacity-testing/spec.md` — `Requirement: Pinned portable scenario authority`, `Requirement: VM-only real-provisioning scope`, and `Requirement: Scenario shapes are mode-neutral` |
| Pinned profile semantic projections across the repository boundary | `openspec/specs/capacity-testing/spec.md` — `Requirement: Pinned profile authority is consumable across repositories` |
...
| Exact one-GPU qualification and measured evidence profiles | `openspec/specs/capacity-testing/spec.md` — `Requirement: Exact one-GPU qualification profile` and `Requirement: Measured one-GPU progression is separately identified` |
| Substantive role receipts and agent-triggered one-shot actions | `openspec/specs/capacity-testing/spec.md` — `Requirement: Substantive portable role receipts` and `Requirement: Agent-triggered frozen actions`; `openspec/specs/capacity-testing/architecture.md` — `Actor and action model` |
| Independent durable VM outcome correlation and reuse | `openspec/specs/capacity-testing/spec.md` — `Requirement: Independent VM-capacity oracle`, `Requirement: Serialized teardown and reuse`, and `Requirement: Every stage restores its declared baseline`; `openspec/specs/capacity-testing/architecture.md` — `Outcome and oracle model` and `Cleanup, reuse, and progression` |
| One-GPU seller topology is globally fenced, and private bindings correlate without publishing enumerable live identities | `openspec/specs/capacity-testing/spec.md` — `Requirement: Seller topology safety`, `Requirement: Logical slots bind privately to live identities`, and `Requirement: Private authority bindings resist identifier enumeration`; `openspec/specs/capacity-testing/architecture.md` — `Public and private ownership` and `Scenario and topology model` |
| Outcome policy is frozen before Q0 and reports independent capacity frontiers | `openspec/specs/capacity-testing/spec.md` — `Requirement: Outcome evaluation policy is frozen before Q0` and `Requirement: Evidence classes and capacity frontiers`; `openspec/specs/capacity-testing/architecture.md` — `Outcome and oracle model` |
| SCM-owned defect fingerprint and immutable sanitized finding evidence | `openspec/specs/capacity-testing/spec.md` — `Requirement: Finding occurrences bind one validated VM result`, `Requirement: SCM derives stable defect identity`, and `Requirement: Finding eligibility is closed and cleanup-gated`; `openspec/specs/capacity-testing/architecture.md` — `Evidence and finding model` |
| Representation-aware privacy, exact evidence-root authority, evidence budgets, and literal occurrence prose | `openspec/specs/capacity-testing/spec.md` — `Requirement: Evidence uses one explicit root and raw-byte authority` and `Requirement: Local finding handoff cannot mutate GitHub` |
| Destination authority follows only the reconciled first-parent branch | `openspec/specs/capacity-testing/spec.md` — `Requirement: Destination authority follows the reconciled first parent` |
| Descriptor-rooted owner-only local handoff, dual locking, authenticated crash recovery, and its same-user limitation | `openspec/specs/capacity-testing/spec.md` — `Requirement: Local finding handoff cannot mutate GitHub`; `openspec/specs/capacity-testing/architecture.md` — `Owner-only handoff and threat boundary` |
| Preparatory public flows have no live-resource or GitHub side effects | `openspec/specs/capacity-testing/spec.md` — `Requirement: Public contract preparation has no live side effects`; `openspec/specs/capacity-testing/architecture.md` — `Current limits`; `docs/development/ARCHITECTURE.md` — `Agent-driven capacity experiments` |
| Evidence labels match exercised boundaries | `openspec/specs/test-compatibility/spec.md` — `Requirement: Evidence labels match the exercised boundary`; `openspec/specs/test-compatibility/architecture.md` — `Agent-driven capacity evidence` |
| Public/private capacity-test authority split and portable local/cloud/Tekton executor seam | `openspec/specs/test-compatibility/spec.md` — `Requirement: Capacity contracts preserve a portable executor seam`; `openspec/specs/test-compatibility/architecture.md` — `Public and private capacity-test authority` and `Pinned semantics and the executor seam`; `openspec/specs/capacity-testing/architecture.md` — `Executor compatibility`; `docs/development/ARCHITECTURE.md` — `Agent-driven capacity experiments` |
