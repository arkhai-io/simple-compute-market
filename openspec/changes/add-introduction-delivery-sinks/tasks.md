...
- [ ] 1.1 Create `kit/delivery` (`market_delivery`) as `arkhai-kit-delivery` 0.1.0
...
- [ ] 1.2 `DeliveryEvent`: event kind, neutral obligation reference, agreement
...
- [ ] 1.3 `DeliverySink` protocol plus the configuration models: a `[Delivery]`
      ...
      representation.
- [ ] 1.4 Sink discovery over the `market.delivery_sinks` entry-point group,
...
- [ ] 1.5 Bounded dispatch shared by both sides: per-sink timeout, never raises to
...
- [ ] 1.6 Evidence: kit unit tests for event construction and rendering over
...
- [ ] 2.1 `file`: append the event to a configured path, bounded and non-fatal.
- [ ] 2.2 `command`: run a configured argument list with no shell, the event on
      standard input, no interpolation of event content into arguments, and a
      timeout.
- [ ] 2.3 `webhook`: POST the event as JSON to one configured URL with a timeout;
      a failure status is a reported failure, not an exception to the caller.
- [ ] 2.4 `smtp`: send one message to configured recipients over the standard
      library client, with the rendered form as the body.
- [ ] 2.5 Evidence: per-sink unit tests, including contact content carrying shell
