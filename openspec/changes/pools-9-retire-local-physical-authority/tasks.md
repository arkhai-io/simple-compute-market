...
- [ ] 7.5 **Roadmap currency.** Update Goal 1's current-state description and
      gap mapping in `docs/development/ROADMAP.md`. The conditional this task
      originally carried is resolved: `add-development-roadmap` landed and was
      archived 2026-08-13, so the roadmap exists and the step is owed
      unconditionally.
