...
- [ ] 1.1 Re-verify `design.md`'s Context measurements before designing against them:
      the eight concerns, their per-domain line counts, and that bare metal still has
      none of them.
- [ ] 1.2 Define where kit-owned storefront runtime sits, what a domain supplies to it,
      and its composition point, extending the injected-contract shape
      `storefront-domain-parameterization` establishes.
- [ ] 1.3 Confirm the dependency direction: kit depends downward only, per
...
- [ ] 2.1 Compare the VM and API-credits implementations line by line and record where
      they already differ. `design.md` names silent drift resolution as the way a
      refactor becomes a behavior change.
- [ ] 2.2 Move the mechanism into kit; leave chain configuration domain-supplied.
- [ ] 2.3 Compose all three domains, including bare metal, which has no implementation
      today. Remove both existing copies in this change.
- [ ] 2.4 Focused tests: kit unit coverage; each domain's chain client construction
      behaves identically to before; bare metal gains the concern.

## 3. Extract `negotiation_watchdog`

- [ ] 3.1 Compare the two implementations and record differences, as in 2.1.
- [ ] 3.2 Move the sweep mechanism into kit; leave the timeout value and terminal-state
      vocabulary domain-supplied.
- [ ] 3.3 Compose all three domains and remove both copies.
- [ ] 3.4 Focused tests: stale threads are marked abandoned on the same schedule as
      before, per domain; bare metal gains the sweep.

## 4. Packaging follow-through

- [ ] 4.1 Update build targets and Dockerfile wheel-refresh entries for the kit package
      gaining modules and the domain wheels losing them.
- [ ] 4.2 Regenerate affected lockfiles and verify they contain no absolute paths — this
...
- [ ] 5.2 Confirm no domain retains a local implementation of either extracted concern —
...
- [ ] 6.2 **Import placement.** Central rather than incidental here: extraction changes
      import direction, and a function-level import retained to dodge a cycle would hide
      a layering violation.
- [ ] 6.3 **Documentation compliance.** Confirm the kit-ownership and no-copy-survives
...
- [ ] 6.5 **Roadmap currency.** Update Goal 4's current-state description and gap
