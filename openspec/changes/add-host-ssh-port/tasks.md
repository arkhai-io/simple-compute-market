...
- [ ] 1.1 Add `ssh_port` to `Host` in
      ...
      independently.
- [ ] 1.2 Add `_migrate_hosts_ssh_port` to
...
- [ ] 1.3 Register it in the `MIGRATIONS` tuple, dated after
      `20260815_001_pool_declared_offering_modes`, which is currently last.
- [ ] 1.4 Note that `check_schema_version` compares against `MIGRATIONS[-1]`,
...
- [ ] 2.1 Add `ssh_port` to `HostCreate`, `HostUpdate`, and `HostResponse` in
...
- [ ] 2.2 Constrain to 1–65535 at the model boundary so an out-of-range value
      returns 400 rather than reaching the database.
- [ ] 2.3 Confirm the create and update paths in
...
- [ ] 3.1 Map `ansible_port` to `ssh_port` in `_parse_ini` in
      `domains/vms/provisioning/adapter/src/vm_provisioning_adapter/services/host_service.py`.
- [ ] 3.2 Correct the docstring's variable table. It currently lists
      ...
      deliberate.
- [ ] 3.3 Reject a malformed or out-of-range `ansible_port`: skip the entry
      ...
      differ deliberately.
- [ ] 3.4 Confirm both INI entry points inherit this: `POST
...
- [ ] 4.1 Emit `ansible_port` from `HostService.render_inventory_ini` in
      `domains/vms/provisioning/adapter/src/vm_provisioning_adapter/services/host_service.py`,
      for every host rather than only when it differs from 22.
- [ ] 4.2 Emit `ansible_port` from `AnsibleService.write_inventory` in
      `domains/vms/provisioning/adapter/src/vm_provisioning_adapter/services/ansible_service.py`.
- [ ] 4.3 These are two separate implementations of the same INI line and they
...
- [ ] 4.4 Confirm the connectivity check (`GET /hosts/{host}/connectivity`),
...
- [ ] 5.1 Parser: `ansible_port` present yields the port; absent yields 22;
      non-integer skips the entry with a warning; `0` and `65536` skip.
- [ ] 5.2 API: create without `ssh_port` yields 22; create with one yields it;
      update changes it; `HostResponse` carries it; out-of-range returns 400.
- [ ] 5.3 Renderers: a non-default port appears in both; the two renderers
      agree; a default port still renders explicitly.
- [ ] 5.4 Migration: applied against a database holding rows created before the
...
- [ ] 5.5 Round trip: import an INI carrying `ansible_port`, render the
...
- [ ] 6.1 Record in `design.md` that appending to `MIGRATIONS` makes
      ...
      or `make migrate`.
- [ ] 6.2 State plainly that this change does not run them. Applying a
      migration to a deployed database is a mutation of persistent state and
      needs its own authorized packet.

## 7. Closeout

- [ ] 7.1 **Comment hygiene.** `make check-comment-hygiene` from the repository
...
- [ ] 7.2 **Import placement.** Check each import added to the touched Python
      modules for a real reason to be local before moving it, and verify any
      move with `make test` rather than a syntax check.
- [ ] 7.3 **Documentation compliance.** Re-read `openspec/README.md`'s
...
- [ ] 7.5 **Roadmap currency.** Decide with the other two changes in this
      campaign whether `docs/development/ROADMAP.md` warrants a goal covering
      reaching hosts without an inbound route, and record the disposition
      either way.
- [ ] 7.6 **Promotion.**

| Accepted decision | Permanent location |
|---|---|
| The host registry is the authority for how the provisioner connects, including port, and every path derives its connection from a rendered inventory | `openspec/specs/physical-provisioning/spec.md` |
| `ansible_port` renders for every host, so a rendered inventory says exactly what the registry holds | `openspec/specs/physical-provisioning/spec.md` |
| A malformed port fails the entry rather than defaulting, because an unreachable host is not a degraded one | `openspec/specs/physical-provisioning/spec.md` |
