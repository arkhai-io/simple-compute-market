...
Contact file publication failures use generic validation messages or fixed stage
and confirmed count, without echoing supplied listing IDs or validation values. Failure SHALL NOT claim uncertain remote
...
- Explicit chart enrollment, reference/privacy guards and mounted path agreement:
  `scripts/tests/test_contact_declarations_chart.py`.
- Rendered configuration through installed general and historical startup loaders:
  `domains/bare_metal/storefront/tests/test_contact_declarations_chart_startup.py`.
