...
Every `tasks.md` for a change or a major section within one must end with a closeout task before implementation is considered planned, not invented after a review round asks for it. The closeout task has nine parts:
...
7. **Documentation citations.** Run `make check-doc-citations CHANGE=<change-name>` and resolve every match. `AGENTS.md` treats a citation whose target does not resolve as a blocking defect rather than a stale link, and this target additionally rejects a citation pointing at a *tombstone*: a tombstoned file still exists on disk while its content is gone, so the plain existence test that preceded this could never fail on a rename-to-tombstone — the likeliest broken citation in a renaming change. Scoped to the change on purpose. A change owes the citations it wrote, and gating archival on the whole repository's documentation debt would let one team's stale runbook block everyone else's, which turns a check into something people route around. Run the target unscoped to see the repository-wide picture.
8. **End-to-end pipeline.** Confirm the end-to-end pipeline passes, and record the evidence — the run, its result, and the scenarios that exercise this change's behaviour. A change is not complete because its own suites are green: this is the tier that catches a wire contract whose two sides disagree, a service that starts cleanly and cannot settle, and a configuration gap no unit test can see, none of which any in-process test will report. If the pipeline cannot run for a reason unrelated to the change, record that as an explicit blocker naming the cause and the change that owns it, and treat the validations it gates as unrun rather than as passed.
9. **Promotion.** Complete the design-promotion record (see below).
