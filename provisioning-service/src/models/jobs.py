...
class ProvisionRequest(BaseModel):
    ...
    vm_os_variant: Optional[str] = Field(
        default=None, description="OS variant for virt-install"
    )

    # --- Tenant SSH (create only, optional) ---
    ssh_pubkey: Optional[str] = Field(
        default=None, description="Tenant SSH public key"
    )
    ...
    vm_gpu_device: Optional[str] = Field(
        default=None, description="Single GPU PCI address"
    )
    vm_gpu_devices: Optional[list[str]] = Field(
        default=None, description="Multiple GPU PCI addresses"
    )
    vm_gpu_partition_size: Optional[str] = Field(
        default=None, description="MIG/SR-IOV partition"
    )
    ...
    frp_dashboard_password: Optional[str] = Field(
        default=None, description="FRP dashboard password"
    )
    ...
    vm_lease_end: Optional[str] = Field(
        default=None, description="UTC datetime 'YYYY-MM-DD HH:MM'"
    )
    ...
    max_retries: Optional[int] = Field(
        default=None,
        description="Max retry attempts (overrides default)",
        ge=0,
        le=10,
    )

    @model_validator(mode="after")
    def validate_action_params(self) -> "ProvisionRequest":
        """Validate required params per action."""
        a = self.vm_action
        if a not in ("list", "check") and not self.vm_target:
        ...
        if a == "create" and self.frp_server_addr and not self.frp_dashboard_password:
            raise ValueError(
                "frp_dashboard_password required when frp_server_addr is set"
            )
        return self
