...
_extract_tenant_user, _extract_ansible_json, _inject_golden_credentials.

start_playbook / wait_for_playbook are thin delegation to AnsibleService
...
def _make_service(
    ...
    ansible_svc = MagicMock()

    return ProvisioningService(settings, ansible_svc)
...
def _base_params(**overrides) -> ProvisioningParams:
    ...
    return ProvisioningParams(**defaults)


def _build(svc: ProvisioningService, **overrides) -> str:
...
class TestBuildVmVarsCreate:
    ...
    def test_os_variant_included_when_set(self):
        svc = _make_service()
        assert "vm_os_variant: ubuntu22.04" in _build(svc, vm_os_variant="ubuntu22.04")
...
# ---------------------------------------------------------------------------
# _build_vm_vars — lease_end action
# ---------------------------------------------------------------------------

class TestBuildVmVarsLease:
    def test_vm_lease_end_quoted(self):
        svc = _make_service()
        ...
        assert 'vm_lease_end: "2025-12-31 23:59"' in yaml

    def test_vm_lease_end_absent_when_none(self):
        svc = _make_service()
...
class TestExtractTenantUser:
    ...
    def test_fallback_generic_pattern(self):
        svc = _make_service()
        output = "ssh -p 5555 someuser@other.host"
        assert svc._extract_tenant_user(output) == "someuser"
...
class TestExtractAnsibleJson:
    ...
    def test_handles_nested_json_correctly(self):
        svc = _make_service()
        output = '"vm_creation_data": {"action": "create", "auth": {"user": "root", "pass": "pw"}}'
        result = svc._extract_ansible_json(output, "create")
        assert result["auth"]["user"] == "root"

    def test_all_action_names_have_mappings(self):
        """Every VM_ACTION defined in models must resolve to a non-None extraction
        when a matching fact is present in the output."""
        svc = _make_service()
        actions = [
