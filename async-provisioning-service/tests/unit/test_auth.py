...
class TestVerifyAgentWithRegistry:
    ...
    async def test_cache_hit_skips_http(self):
        """Second call with same agent uses cache, no HTTP request."""
        response = MagicMock(status_code=200)
        response.json.return_value = {"status": "healthy"}
        mock_client = _make_mock_client(response=response)

        with patch("async_provisioning_service.api.auth.httpx.AsyncClient", return_value=mock_client):
            result1 = await verify_agent_with_registry(REGISTRY_URL, VALID_AGENT_1)
            result2 = await verify_agent_with_registry(REGISTRY_URL, VALID_AGENT_1)

        assert result1 is True
        assert result2 is True
        # HTTP call was only made once (second was cache hit)
        assert mock_client.get.call_count == 1

    @pytest.mark.anyio
    async def test_non_200_404_fails_open(self):
        """Registry returning 500 -> fails open (returns True)."""
...
class TestAgentAuthMiddleware:
    ...
    @pytest.mark.anyio
    async def test_post_registry_rejects_returns_403(self):
        """Registry URL set, verify_agent_with_registry returns False -> 403."""
        with patch(
            "async_provisioning_service.api.auth.verify_agent_with_registry",
            AsyncMock(return_value=False),
        ) as mock_verify:
            resp = await _asgi_request(
                _make_test_app(auth_enabled=True, registry_url=REGISTRY_URL),
                "post",
                "/api/v1/jobs",
                json={"ssh_pubkey": "ssh-rsa AAA"},
                headers={"X-Agent-ID": VALID_AGENT_1},
    ...
    @pytest.mark.anyio
    async def test_post_registry_accepts_returns_200(self):
        """Registry URL set, verify_agent_with_registry returns True -> 200."""
        with patch(
            "async_provisioning_service.api.auth.verify_agent_with_registry",
