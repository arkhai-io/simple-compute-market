...
def test_provisioning_e2e():
    ...
    with httpx.Client(timeout=30) as client:
        response = client.post(f"{base_url}/api/v1/jobs", json=request_payload)
        ...
        while time.time() < deadline and status in {"queued", "running"}:
            status_response = client.get(f"{base_url}/api/v1/jobs/{job_id}")
        ...
        logs_response = client.get(f"{base_url}/api/v1/jobs/{job_id}/logs")
